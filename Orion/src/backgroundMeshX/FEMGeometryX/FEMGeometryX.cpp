...
/*!****************************************************************************/
/*!****************************************************************************/
FEMGeometryX::FEMGeometryX(InputFileData* InputData,
...
//    setVolumeGaussPoints(InputData,modelData,logFile);
//
//    // Calculate the boundary gauss points.
//    setBoundGaussPoints(InputData,modelData,logFile);
//
...
//
////       setGlobalFEShapeFuncDerivs(InputData,modelData,logFile);
//
//  }
//---------------------------------------------------------------------
...
//    setPtcleWeightsLoads(InputData,modelData,logFile);
//
...
//    // loads.
//    setBoundPtcleWeightsLoads(InputData,modelData,logFile);
//
//  }
}

FEMGeometryX::~FEMGeometryX() {
...
void FEMGeometryX::readMeshFile(InputFileData* InputData,
		std::map<std::string, double>& modelData, std::ifstream& meshFile,
		std::ofstream& logFile) {
	...
	// -----------------------------------------------
	logFile <<"In FEMGeometryX::readMeshFile "<< endl;
	// -----------------------------------------------
	...
	particles = vector<ParticleX>(nodesNum, ParticleX(usedDOF));
	...
	if (globalElemNum == 0) {
		logFile
				<< "In FEMGeometryX::readMeshFile no volume elements in 'mesh.dat'.\n"
	...
	vector<FEMElementX> allNodesElements(globalElemNum, FEMElementX(usedDOF));
	nodesElements.resize(globalElemNum, FEMElementX(usedDOF));
	...
	for (int i = 0; i < globalElemNum; i++) {
		...
		if (matID > (int) InputData->getNumberOfMats() || matID == 0) {
			logFile
					<< "In FEMGeometryX::readMeshFile matID in mesh file 'mesh.dat'\n"
		...
		if (i == 0)
			...
			logFile << "In FEMGeometryX::readMeshFile elemType must be uniform\n"
		...
		nodesElements[i].setSurfaceElems(surfaceIDGenerator(tempMat,InputData,modelData,logFile)) ;
		...
		if (abs(elemType) > 2) {
			logFile << "In FEMGeometryX::readMeshFile elemType " << elemType
...
//void FEMGeometryX::storeElements(InputFileData* InputData,
//		std::vector<FEMElement>& allNodesElements,
//		std::map<std::string, double>& modelData, std::ofstream& logFile) {
//
//	using namespace std;
//
//	cout << "In FEMGeometryX::storeElements " << endl;
//
...
//      if(position != -1) {
//	logFile<<"In function FEMGeometryX::storeElements repetetive "
...
// particles.
//void FEMGeometryX::matchElemNodeOrder(InputFileData* InputData,
//		std::vector<FEMElement>& allNodesElements,
//		std::vector<int>& oldGlobElemIdx, std::vector<int>& newGlobElemIdx,
//		std::map<std::string, double>& modelData, std::ofstream& logFile) {
//
//	using namespace std;
//
//	cout << "In FEMGeometryX::matchElemNodeOrder " << endl;
//
//	int rank, size;
//	MPI_Comm_rank(MPI_COMM_WORLD, &rank);
//	MPI_Comm_size(MPI_COMM_WORLD, &size);
...
// arrange the elements in the new ordering and store only the
// local elements
//void FEMGeometryX::rearrangeElements(InputFileData* InputData,
//		std::vector<FEMElement>& allNodesElements, intVector& oldGlobElemIdx,
//		std::map<std::string, double>& modelData, std::ofstream& logFile) {
//
//	using namespace std;
//
//	cout << "In FEMGeometryX::rearrangeElements " << endl;
//
//	int usedDOF = (int) modelData["usedDegreesOfFreedom"];
//	int globalElemNum = allNodesElements.size();
//
...
//		localElemNum = 0;
//		logFile << "In function FEMGeometryX::rearrangeElements " << localElemNum
...
//			intVector& globalNodes =
//					allNodesElements[oldGlobElemIdx[i]].getNodes();
...
//bool FEMGeometryX::swapVolToFaceElemInfo(InputFileData* InputData,
//		FEMElement& sElem, std::map<std::string, double>& modelData,
//		std::ofstream& logFile) {
//
//	using namespace std;
//
//	cout << "In FEMGeometryX::swapVolToFaceElemInfo " << endl;
//
...
/*!****************************************************************************/
// Create boundary elements.
//void FEMGeometryX::createBoundElems(InputFileData* InputData,
//		std::map<std::string, double>& modelData, std::ofstream& logFile) {
//
//	using namespace std;
//
//	cout << "In FEMGeometryX::createBoundElems " << endl;
//
...
/*!****************************************************************************/
// Create surface elements and store their surface load.
//void FEMGeometryX::createSurfaceElems(InputFileData* InputData,
//		std::map<std::string, double>& modelData, std::ofstream& logFile) {
//
//	using namespace std;
//
//	cout << "In FEMGeometryX::createSurfaceElems " << endl;
//
//	std::map<std::string, double>& backGroundMeshInfo =
//			InputData->getBackGroundMeshInfo();
//
...
//			InputData->getSurfaceDepolarisationBoundConds();
...
//			+ momentLoads.size() + electricChargeLoads.size()
//			+ defBoundConds.size();
//
//	surfaceElemNum += depolarisationBoundConds.size()
//			+ electricBoundConds.size() + microBoundConds.size();
//
...
//
//		pos = findIntVecPos(elem, 0, boundaryElemIdx.size(), boundaryElemIdx);
//
//		if (pos != -1) {
//			logFile << "In function FEMGeometryX::createSurfaceElems repetetive "
...
//
//			for (int j = nodesPerSurfaceElem + 1; j < tractionLoads[i].size();
//					j += 2) {
...
//					modelData, logFile)) {
//				logFile
//						<< "In function FEMGeometryX::createSurfaceElems no corresponding\n"
//						<< "volume element can be found for surface element '"
//						<< elem << "'!" << endl;
//				MPI_Abort(MPI_COMM_WORLD, 1);
//			}
//
...
//			double& pressure =
//					surfaceNodesElems[currentIdx].getSurfacePressure();
...
//			if (pos != -1) {
//				logFile
//						<< "In function FEMGeometryX::createSurfaceElems repetetive "
...
//			blVector& boundDOF =
//					surfaceNodesElems[currentIdx].getSurfaceMomentDOF();
//			dbVector& moment = surfaceNodesElems[currentIdx].getSurfaceMoment();
//			boundDOF.resize(usedDims);
//			moment.resize(usedDims);
//
//			for (int j = nodesPerSurfaceElem + 1; j < momentLoads[i].size();
//					j += 2) {
...
//			blVector& boundDOF =
//					surfaceNodesElems[currentIdx].getSurfaceElectricChargeDOF();
//			dbVector& loads =
//					surfaceNodesElems[currentIdx].getSurfaceElectricCharge();
//			boundDOF.resize(1);
//			loads.resize(1);
//
//			for (int j = nodesPerSurfaceElem + 1;
//					j < electricChargeLoads[i].size(); j += 2) {
...
//
//			if (dispDOF <= 0) {
//				logFile << "In function FEMGeometryX::createSurfaceElems "
//						<< "displacement boundary conditions are not supported!"
//						<< endl;
//				break;
//				//MPI_Abort(MPI_COMM_WORLD,1);
//			}
...
//				// and order (linear/quadratic)
//
//				if (!swapVolToFaceElemInfo(InputData,
//						surfaceNodesElems[currentIdx], modelData, logFile)) {
//					logFile
//							<< "In function FEMGeometryX::createSurfaceElems no corresponding\n"
//							<< "volume element can be found for surface element '"
//							<< elem << "'!" << endl;
...
//						surfaceDispBoundElemIdx.size(), 0,
...
//			if (rotDOF <= 0) {
//				logFile << "In function FEMGeometryX::createSurfaceElems "
//						<< "rotation boundary conditions are not supported!"
//						<< endl;
//				break;
//				//MPI_Abort(MPI_COMM_WORLD,1);
//			}
...
//				// Loop over all affected degrees of current element.
//				for (int j = nodesPerSurfaceElem + 1;
//						j < electricBoundConds[i].size(); j += 2) {
...
//
//				for (int j = 0; j < nodesPerSurfaceElem; j++)
//					nodes[j] = newIdx[(int) electricBoundConds[i][j + 1] - 1]
//							+ 1;
//
//				// set the element's material, type (tetrahedra,cube)
//				// and order (linear/quadratic)
...
//				// applied to current elment.
//				pos = findIntMatPos(currentIdx, 0,
//						surfaceElectricBoundElemIdx.size(), 0,
...
//				// Loop over all affected degrees of current element.
//				for (int j = nodesPerSurfaceElem + 1;
//						j < depolarisationBoundConds[i].size(); j += 2) {
...
//
//				for (int j = 0; j < nodesPerSurfaceElem; j++)
//					nodes[j] = newIdx[(int) depolarisationBoundConds[i][j + 1]
//							- 1] + 1;
//
//				// set the element's material, type (tetrahedra,cube)
//				// and order (linear/quadratic)
...
//							boundDOF[dof] = 1;
//							boundConds[dof] =
//									depolarisationBoundConds[i][j + 1];
//						} else {
//							boundDOF.resize(depolarisationDOF + usedDims);
//							boundConds.resize(depolarisationDOF + usedDims);
//
...
//				// applied to current elment.
//				pos = findIntMatPos(currentIdx, 0,
//						surfaceMicroBoundElemIdx.size(), 0,
...
//		logFile << endl;
//	}
//	logFile << "*********** surface electric charge loads *************"
//			<< endl;
//	for (int i = 0; i < surfaceElectricChargeElemIdx.size(); i++) {
//		int& idx = surfaceElectricChargeElemIdx[i][0];
//		blVector& affectedDOF =
//				surfaceNodesElems[idx].getSurfaceElectricChargeDOF();
//		dbVector& conditions =
//				surfaceNodesElems[idx].getSurfaceElectricCharge();
//		logFile << "SURFACE ELEMENT " << surfaceNodesElems[idx].getGlobalID()
//				<< ": ";
//		for (int j = 0; j < conditions.size(); j++)
//			if (affectedDOF[j])
...
/*!****************************************************************************/
// Create line elements and store their line load.
//void FEMGeometryX::createLineElems(InputFileData* InputData,
//		std::map<std::string, double>& modelData, std::ofstream& logFile) {
//
//	using namespace std;
//
//	cout << "In FEMGeometryX::createLineElems " << endl;
//
//	std::map<std::string, double>& backGroundMeshInfo =
//			InputData->getBackGroundMeshInfo();
//
...
//	dbMatrix& depolarisationBoundConds =
//			InputData->getLineDepolarisationBoundConds();
//
//	/*!****************************************************************************/
//	// delete not used boundary conditions
//	if (rotDOF == 0) {
...
//			+ rotBoundConds.size() + electricBoundConds.size()
//			+ depolarisationBoundConds.size();
//
//	if (lineMomentLoads.size() > 0) {
//		logFile << "In function FEMGeometryX::createLineElems "
...
//					modelData, logFile)) {
//				logFile
//						<< "In function FEMGeometryX::createLineElems no corresponding\n"
//						<< "volume element can be found for surface element '"
//						<< elem << "'!" << endl;
//				MPI_Abort(MPI_COMM_WORLD, 1);
//			}
...
//
//				// Loop over all affected degrees of current element.
//				for (int j = startIdxConds; j < dispBoundConds[i].size(); j +=
//						2) {
//					dof = (int) dispBoundConds[i][j] - 1;
//					boundDOF[dof] = true;
//					boundConds[dof] = dispBoundConds[i][j + 1];
...
//				// Store the normal vector.
//				dbVector& normal =
//						lineNodesElems[currentIdx].getSurfaceNormal();
//				normal.resize(usedDims);
//
...
//
//				if (!swapVolToFaceElemInfo(InputData,
//						lineNodesElems[currentIdx], modelData, logFile)) {
//					logFile
//							<< "In function FEMGeometryX::createLineElems no corresponding\n"
//							<< "volume element can be found for surface element '"
//							<< elem << "'!" << endl;
...
//				if (pos != -1) {
//					logFile
//							<< "In function FEMGeometryX::createLineElems repetetive "
...
//
//					// Loop over all affected degrees of current element.
//					for (int j = startIdxConds; j < dispBoundConds[i].size();
//							j += 2) {
//						dof = (int) dispBoundConds[i][j] - 1;
//						boundDOF[dof] = 1;
//						boundConds[dof] = dispBoundConds[i][j + 1];
...
//
//				// Loop over all affected degrees of current element.
//				for (int j = startIdxConds; j < rotBoundConds[i].size(); j +=
//						2) {
//					dof = (int) rotBoundConds[i][j] - 1;
//					boundDOF[dispDOF + dof] = 1;
//					boundConds[dispDOF + dof] = rotBoundConds[i][j + 1];
...
//
//					// Loop over all affected degrees of current element.
//					for (int j = startIdxConds; j < rotBoundConds[i].size();
//							j += 2) {
//						dof = (int) rotBoundConds[i][j] - 1;
//						boundDOF[dispDOF + dof] = 1;
//						boundConds[dispDOF + dof] = rotBoundConds[i][j + 1];
...
//				// applied to current elment.
//				pos = findIntMatPos(currentIdx, 0,
//						lineElectricBoundElemIdx.size(), 0,
...
//		blVector& affectedDOF = lineNodesElems[idx].getLineElectricBoundDOF();
//		dbVector& conditions = lineNodesElems[idx].getLineElectricBoundConds();
//		dbVector& normal = lineNodesElems[idx].getSurfaceNormal();
//		logFile << "LINE ELEMENT " << lineNodesElems[i].getGlobalID() << ": ";
//		for (int j = 0; j < conditions.size(); j++)
//			if (affectedDOF[j])
...
//void FEMGeometryX::rearrangeMeshConfiguration(InputFileData* InputData,
//					     std::map<std::string,double>& modelData,
//					     std::ofstream& logFile) {
//
//  using namespace std;
//
//  cout << "In FEMGeometryX::rearrangeMeshConfiguration " <<endl;
//
//  int usedDims = (int)modelData["usedDimensions"];
//  int usedDOF = (int)modelData["usedDegreesOfFreedom"];
//
...
//			     vElemIdx);
//    if(position == -1) {
//      logFile<<"In FEMGeometryX::rearrangeMeshConfiguration "
...
// Ensure that all elements have the necessary approximation and
// integration tools assigned.
//void FEMGeometryX::setApproxAndIntTools(InputFileData* InputData,
//				       std::map<std::string,double>& modelData,
//				       std::ofstream& logFile) {
//
//  using namespace std;
//
//  cout << "In FEMGeometryX::setApproxAndIntTools " <<endl;
//
//  std::map<std::string,double>& backGroundMeshInfo =
//    InputData->getBackGroundMeshInfo();
//
...
//void FEMGeometryX::setVolumeGaussPoints(InputFileData* InputData,
//				       std::map<std::string,double>& modelData,
//				       std::ofstream& logFile) {
//  using namespace std;
//
//  cout << "In FEMGeometryX::setVolumeGaussPoints " <<endl;
//
//  int usedDims = (int)modelData["usedDimensions"];
//  int usedDOF = (int)modelData["usedDegreesOfFreedom"];
//
...
//	   <<gaussPoints[i].getCoord(2)<<" matID "<<matID<<endl;
//  }
//  logFile<<"*****************************************************"<<endl;
...
//void FEMGeometryX::setBoundGaussPoints(InputFileData* InputData,
//				      std::map<std::string,double>& modelData,
//				      std::ofstream& logFile) {
//  using namespace std;
//
//  cout << "In FEMGeometryX::setBoundGaussPoints " <<endl;
//
...
//void FEMGeometryX::setSurfaceGaussPoints(InputFileData* InputData,
//					std::map<std::string,double>& modelData,
//					std::ofstream& logFile) {
//
//  using namespace std;
//
//  cout << "In FEMGeometryX::setSurfaceGaussPoints " <<endl;
//
//  int usedDims = (int)modelData["usedDimensions"];
//  int usedDOF = (int)modelData["usedDegreesOfFreedom"];
//  int defDOF = (int)modelData["deformationDegreesOfFreedom"];
...
// Calculate the surface gauss points coordinate's and store the
// surface load if applied.
//void FEMGeometryX::setLineGaussPoints(InputFileData* InputData,
//				     std::map<std::string,double>& modelData,
//				     std::ofstream& logFile) {
//
//  using namespace std;
//
//  cout << "In FEMGeometryX::setLineGaussPoints " <<endl;
//
//  int usedDims = (int)modelData["usedDimensions"];
//  int usedDOF = (int)modelData["usedDegreesOfFreedom"];
//  int defDOF = (int)modelData["deformationDegreesOfFreedom"];
...
//void FEMGeometryX::setLineSurfaceGPoints(InputFileData* InputData,
//					std::map<std::string,double>& modelData,
//					std::ofstream& logFile) {
//
//  using namespace std;
//
//  cout << "In FEMGeometryX::setLineSurfaceGPoints " <<endl;
//
//  int usedDims = (int)modelData["usedDimensions"];
//  int usedDOF = (int)modelData["usedDegreesOfFreedom"];
//  int defDOF = (int)modelData["deformationDegreesOfFreedom"];
...
//      default:
//
//	logFile<<"In FEMGeometryX::setLineSurfaceGPoints node ID "<<k
...
//  if(integrationMethod != 1) {
//
//    logFile<<"In FEMGeometryX::setLineSurfaceGPoints integration method\n"
//	   <<"is not supported!"<<endl;
//    MPI_Abort(MPI_COMM_WORLD,1);
//
//  }
...
//  //logFile<<"In function FEMGeometryX::setLineGaussPoints "
...
//void FEMGeometryX::setVolumeGaussWeights(InputFileData* InputData,
//					std::map<std::string,double>& modelData,
//					std::ofstream& logFile) {
//
//
//  using namespace std;
//
//  cout << "In FEMGeometryX::setVolumeGaussWeights " <<endl;
//
//  // Loop over all local elements.
//  for(int i=0;i<nodesElements.size();i++) {
//    FEMElement& elem = nodesElements[i];
...
// surface normal vector.
//void FEMGeometryX::setSGaussWeightsNormals(InputFileData* InputData,
//					  std::map<std::string,double>& modelData,
//					  std::ofstream& logFile) {
//
//  using namespace std;
//
//  cout << "In FEMGeometryX::setSGaussWeightsNormals " <<endl;
//
//  int usedDims = (int)modelData["usedDimensions"];
//
...
//  int startIdx,endIdx;
//
//  intMatrix surfaceGaussPtsIdx;
//  getAllSurfaceGaussPtsIdx(surfaceGaussPtsIdx,logFile);
//
...
//void FEMGeometryX::setLineGaussPtsWeights(InputFileData* InputData,
//					 std::map<std::string,double>& modelData,
//					 std::ofstream& logFile) {
//
//  using namespace std;
//
//  cout << "In FEMGeometryX::setLineGaussPtsWeights " <<endl;
//
//  // Determine the a local portion of boundary elements for calculating.
//  int rank,size;
//  MPI_Comm_rank(MPI_COMM_WORLD,&rank);
//  MPI_Comm_size(MPI_COMM_WORLD,&size);
//
//  intMatrix lineGaussPtsIdx;
//  getAllLineGaussPtsIdx(lineGaussPtsIdx,logFile);
//
...
//void FEMGeometryX::setAllPtclsWeights(InputFileData* InputData,
//				     std::ofstream& logFile) {
//
//  using namespace std;
//
//  cout << "In FEMGeometryX::setAllPtclsWeights " <<endl;
//
//#ifdef _FEdebugMode_
//  logFile<<"#####################################################"<<endl;
//#endif
...
//
//      else {
//	logFile<<"In FEMGeometryX::setAllPtclsWeights element type "
...
//
//    if(globalElemSize == 0) {
//      logFile<<"In FEMGeometryX::setAllPtclsWeights particle "<<i
...
//void FEMGeometryX::setPtcleWeightsLoads(InputFileData* InputData,
//				       std::map<std::string,double>& modelData,
//				       std::ofstream& logFile) {
//
//  using namespace std;
//
//  cout << "In FEMGeometryX::setPtcleWeightsLoads " <<endl;
//
//  int usedDOF = (int)modelData["usedDegreesOfFreedom"];
//  int usedDims = (int)modelData["usedDimensions"];
//
...
//
//	      logFile<<"In FEMGeometryX::setPtcleWeightsLoads non-"
...
//	logFile<<"In function FEMGeometryX::setPtcleWeightsLoads "
...
// applied.
//  void FEMGeometryX::setBoundPtcleWeightsLoads(InputFileData* InputData,
//					    std::map<std::string,double>& modelData,
//					    std::ofstream& logFile) {
//
//  using namespace std;
//
//  cout << "In FEMGeometryX::setBoundPtcleWeightsLoads " <<endl;
//
...
//  // are applied.
//  setLinePtcleWeightsLoads(InputData,modelData,logFile);
//
...
//void FEMGeometryX::setBoundPointIntegration(InputFileData* InputData,
//					   std::map<std::string,double>& modelData,
//					   std::ofstream& logFile) {
//
//  using namespace std;
//
//  cout << "In FEMGeometryX::setBoundPointIntegration " <<endl;
//
//  int usedDims = (int)modelData["usedDimensions"];
//  int usedDOF = (int)modelData["usedDegreesOfFreedom"];
//  int defDOF = (int)modelData["deformationDegreesOfFreedom"];
...
//    logFile<<"In function FEMGeometryX::setBoundPointIntegration "
...
//		    conditions[conditionID][idx] != pointDispBounds[i][k+1]) {
//	      logFile<<"In FEMGeometryX::setBoundPointIntegration non-constant\n"
...
// a line boundary has to be integrated and store the line loads if any
// are applied.
//void FEMGeometryX::setLinePtcleWeightsLoads(InputFileData* InputData,
//					   std::map<std::string,double>& modelData,
//					   std::ofstream& logFile) {
//
//  using namespace std;
//
//  cout << "In FEMGeometryX::setLinePtcleWeightsLoads " <<endl;
//
//  // Compute all line Gauss points and store their line load.
//  if(lineNodesElems.size() > 0) {
//    logFile<<"In FEMGeometryX::setLinePtcleWeightsLoads line boundary \n"
...
// the boundary has to be integrated and store the surface loads if any
// are applied.
//void FEMGeometryX::setSurfacePtcleWeightsLoads(InputFileData* InputData,
//					      std::map<std::string,double>& modelData,
//					      std::ofstream& logFile) {
//
//  using namespace std;
//
//  cout << "In FEMGeometryX::setSurfacePtcleWeightsLoads " <<endl;
//
//  int usedDOF = (int)modelData["usedDegreesOfFreedom"];
//  int usedDims = (int)modelData["usedDimensions"];
//  int defDOF = (int)modelData["deformationDegreesOfFreedom"];
...
//		logFile<<"In FEMGeometryX::setSurfacePtcleWeightsLoads non-constant\n"
//		       <<"conditions are not supported!"
//		       << endl;
//		MPI_Abort(MPI_COMM_WORLD,1);
//	      }
...
//    logFile<<"In function FEMGeometryX::surfacePtcleWeightsLoads "
...
//void FEMGeometryX::getAllSurfaceGaussPtsIdx(intMatrix& mat,
//					   std::ofstream& logFile) {
//
//  using namespace std;
//
//  cout << "In FEMGeometryX::getAllSurfaceGaussPtsIdx " <<endl;
//
//  mat.resize(0);
//
...
//void FEMGeometryX::getAllLineGaussPtsIdx(intMatrix& mat,
//		std::ofstream& logFile) {
//
//	using namespace std;
//
//	cout << "In FEMGeometryX::getAllLineGaussPtsIdx " << endl;
//
//	mat.resize(0);
//
...
ElementTemplateX* FEMGeometryX::getVolumeElementTemplate(InputFileData* InputData,
		FEMElementX& elem, std::ofstream& logFile) {

	using namespace std;

	cout << "In FEMGeometryX::getVolumeElementTemplate " << endl;
	...
	// Key was not found.
		...
		case 1:
			...
			case 4:
				volumeElemTemplates[position] = new Tetra4ElementTemplateX();
				break;

			default:
				logFile
						<< "In function FEMGeometryX::getVolumeElementTemplate element "
						<< nodesPerElem
						<< " nodes per element are not supported!" << endl;
				MPI_Abort(MPI_COMM_WORLD, 1);
			}
		...
		case 2:
			...
			case 8:
				volumeElemTemplates[position] = new Cube8ElementTemplateX();
				break;

			case 27:
				volumeElemTemplates[position] = new Cube27ElementTemplateX();
				break;
...
ElementTemplateX* FEMGeometryX::getSurfaceElementTemplate(
		InputFileData* InputData, FEMElementX& elem, std::ofstream& logFile) {

	using namespace std;

	cout << "In FEMGeometryX::getSurfaceElementTemplate " << endl;
	...
	// Key was not found.
		...
		case 1:
			...
			case 3:
				surfaceElemTemplates[position] = new Tria3ElementTemplateX();

				break;

			default:
				logFile
						<< "In function FEMGeometryX::getSurfaceElementTemplate element "
						<< nodesPerElem
						<< " nodes per element are not supported!" << endl;
				MPI_Abort(MPI_COMM_WORLD, 1);
			}
		...
		case 2:
			...
			case 4:
				surfaceElemTemplates[position] = new Rect4ElementTemplateX();
				break;

			case 9:
				surfaceElemTemplates[position] = new Rect9ElementTemplateX();
				break;
...
ElementTemplateX* FEMGeometryX::getLineElementTemplate(InputFileData* InputData,
		FEMElementX& elem, std::ofstream& logFile) {

	using namespace std;

	cout << "In FEMGeometryX::getLineElementTemplate " << endl;
	...
	// Key was not found.
		...
		case 2:
			lineElemTemplates[position] = new Line2ElementTemplateX();
			break;

		case 3:
			lineElemTemplates[position] = new Line3ElementTemplateX();
			break;

		default:
			logFile
					<< "In function FEMGeometryX::getLineElementTemplate element "
					<< nodesPerElem << " nodes per element are not supported!"
					<< endl;
			MPI_Abort(MPI_COMM_WORLD, 1);
		}
...
/************************************************************************/
/************************************************************************/
//dbMatrix& FEMGeometryX::getVolumeFEShapeFunctions(InputFileData* InputData,
//		FEMElement& elem, std::ofstream& logFile) {
//
//	using namespace std;
//
//	cout << "In FEMGeometryX::getVolumeFEShapeFunctions " << endl;
//
//	int& elemType = elem.getElemType();
//	intVector& nodes = elem.getNodes();
//	intVector& intPts = elem.getVolumeIntegrationPts();
...
//				volumeFEShapeFuncSets[position], logFile);
//
...
/************************************************************************/
/************************************************************************/
//dbMatrix3& FEMGeometryX::getVolumeFEShapeFuncDerivs(InputFileData* InputData,
//		FEMElement& elem, std::ofstream& logFile) {
//
//	using namespace std;
//
//	cout << "In FEMGeometryX::getVolumeFEShapeFuncDerivs " << endl;
//
//	int& elemType = elem.getElemType();
//	intVector& nodes = elem.getNodes();
//	intVector& intPts = elem.getVolumeIntegrationPts();
...
/************************************************************************/
/************************************************************************/
//dbMatrix& FEMGeometryX::getSurfaceFEShapeFunctions(InputFileData* InputData,
//		FEMElement& elem, std::ofstream& logFile) {
//	using namespace std;
//
//	cout << "In FEMGeometryX::getSurfaceFEShapeFunctions " << endl;
//
//	int& elemType = elem.getElemType();
//	intVector& nodes = elem.getNodes();
//	intVector& intPts = elem.getSurfaceIntegrationPts();
...
//				surfaceFEShapeFuncSets[position], logFile);
//
...
/************************************************************************/
/************************************************************************/
//dbMatrix& FEMGeometryX::getLineFEShapeFunctions(InputFileData* InputData,
//		FEMElement& elem, std::ofstream& logFile) {
//	using namespace std;
//
//	cout << "In FEMGeometryX::getLineFEShapeFunctions " << endl;
//
//	intVector& nodes = elem.getNodes();
//	intVector& intPts = elem.getLineIntegrationPts();
//	int& elemType = elem.getElemType();
...
//void FEMGeometryX::setVolumeFEShapeFunctions(InputFileData* InputData,
//		FEMElement& elem, dbMatrix& sFuncs, std::ofstream& logFile) {
//
//	using namespace std;
//
//	cout << "In FEMGeometryX::setVolumeFEShapeFunctions " << endl;
//
//	intVector& nodes = elem.getNodes();
//	intVector& intPts = elem.getVolumeIntegrationPts();
//	int& elemType = elem.getElemType();
...
//			if (i == j)
//				logFile << "N" << j << "(x_" << i << ") = "
//						<< FEVolumeSet->N(j, FEVolumeSet->nodalCoords[i])
//						<< endl;
...
//void FEMGeometryX::setVolumeFEShapeFuncDerivs(InputFileData* InputData,
//		FEMElement& elem, dbMatrix3& sFuncs, std::ofstream& logFile) {
//
//	using namespace std;
//
//	cout << "In FEMGeometryX::setVolumeFEShapeFuncDerivs " << endl;
//
//	intVector& nodes = elem.getNodes();
//	intVector& intPts = elem.getVolumeIntegrationPts();
//	int& elemType = elem.getElemType();
...
//			for (int j = 0; j < nodesPerElem; j++)
//
//				sFuncs[k][i][j] = FEVolumeSet->dN(j, k,
//						VolumeGaussSet->coord[i]);
//
//#ifdef _FEdebugMode_
//	logFile << "******************************************************" << endl;
...
//void FEMGeometryX::setSurfaceFEShapeFunctions(InputFileData* InputData,
//		FEMElement& elem, dbMatrix& sFuncs, std::ofstream& logFile) {
//	using namespace std;
//
//	cout << "In FEMGeometryX::setSurfaceFEShapeFunctions " << endl;
//
//	intVector& nodes = elem.getNodes();
//	intVector& intPts = elem.getSurfaceIntegrationPts();
//	int& elemType = elem.getElemType();
...
//			if (i == j)
//				logFile << "N_" << j << "(x_" << i << ") = "
//						<< FESurfaceSet->N(j, FESurfaceSet->nodalCoords[i])
//						<< endl;
...
//void FEMGeometryX::setLineFEShapeFunctions(InputFileData* InputData,
//		FEMElement& elem, dbMatrix& sFuncs, std::ofstream& logFile) {
//	using namespace std;
//
//	cout << "In FEMGeometryX::setLineFEShapeFunctions " << endl;
//
//	intVector& nodes = elem.getNodes();
//	intVector& intPts = elem.getLineIntegrationPts();
//	int& elemType = elem.getElemType();
...
//GaussPointSet* FEMGeometryX::getVolumeGaussSet(InputFileData* InputData,
//					      FEMElement& elem,
//					      std::ofstream& logFile) {
//
//  using namespace std;
//
//  cout << "In FEMGeometryX::getVolumeGaussSet " <<endl;
//
//  intVector& intPts = elem.getVolumeIntegrationPts();
//  int gaussPtsPerElem = intPts.size();
//  int& elemType = elem.getElemType();
...
//
//      default:
//	logFile<<"In function FEMGeometryX::getVolumeGaussSet element\n "
...
//GaussPointSet* FEMGeometryX::getSurfaceGaussSet(InputFileData* InputData,
//					       FEMElement& elem,
//					       std::ofstream& logFile) {
//
//  using namespace std;
//
//  cout << "In FEMGeometryX::getSurfaceGaussSet " <<endl;
//
//  intVector& intPts = elem.getSurfaceIntegrationPts();
//  int gaussPtsPerElem = intPts.size();
//  int& elemType = elem.getElemType();
...
//
//      default:
//	logFile<<"In function FEMGeometryX::getSurfaceGaussSet element\n "
...
//GaussPointSet* FEMGeometryX::getLineGaussSet(InputFileData* InputData,
//					    FEMElement& elem,
//					    std::ofstream& logFile) {
//
//  using namespace std;
//
//  cout << "In FEMGeometryX::getLineGaussSet " <<endl;
//
//  intVector& intPts = elem.getLineIntegrationPts();
//  int gaussPtsPerElem = intPts.size();
//
...
//
//      default:
//	logFile<<"In function FEMGeometryX::getLineGaussSet element\n "
...
//void FEMGeometryX::setGlobalFEShapeFuncDerivs(InputFileData* InputData,
//					     std::map<std::string,double>& modelData,
//					     std::ofstream& logFile) {
//
//  using namespace std;
//
//  cout << "In FEMGeometryX::setGlobalFEShapeFuncDerivs " <<endl;
//
//  int usedDims = (int)modelData["usedDimensions"];
//  int usedDOF = (int)modelData["usedDegreesOfFreedom"];
//
//  int idx;
//  dbMatrix jacobi,jacobiInv;
//
//#ifdef _FEdebugMode_
//  logFile<<"#####################################################"<<endl;
...
//
//      // compute dL_s/dx_r
//      FEVolumeSet->jacobiMatrix(particles,nodes,
//				VolumeGaussSet->coord[j],
//				jacobi,logFile);
//
//      calcInvDouble(jacobi,jacobiInv,logFile);
//
//#ifdef _FEdebugMode_
//      logFile<<"---------------- jacobi matrix inv ---------------"<<endl;
//      for(int v=0;v<jacobiInv.size();v++)
//	for(int w=0;w<jacobiInv[v].size();w++)
//	  logFile<<"J["<<v<<"]["<<w<<"]="<<jacobi[v][w]<<endl;
//#endif
//
...
//
//	    feShapeDerivs[r][k] += sFuncs[s][j][k]*jacobiInv[s][r];
//
//
//    }
//
...
/*!****************************************************************************/
// Create surface elements and store their surface load.
//void FEMGeometryX::createAllSurfaceElems(InputFileData* InputData,
//		std::map<std::string, double>& modelData, std::ofstream& logFile) {
//
//	using namespace std;
//
...
void FEMGeometryX::combiningSurfaceVolumeElems(InputFileData* InputData,
		std::map<std::string, double>& modelData, std::ofstream& logFile) {
...
/*!****************************************************************************/
//! Define the surface elements in the volume element list.
intMatrix FEMGeometryX::decompVolumeToSurfaceElems(intVector& volNodes,
	...
	default:
		cout
				<< "In FEMGeometryX::decompVolumeToSurfaceElems: Element"
...
intMatrix FEMGeometryX::decompTetraToTrianElems(intVector& volNodes,
...
intMatrix FEMGeometryX::decompHexaToQuadElems(intVector& volNodes,
		InputFileData* InputData, std::map<std::string, double>& modelData,
		std::ofstream& logFile) {
	...
	if (volNodes.size() != 8) {
		logFile
				<< "In FEMGeometryX::decompHexaToQuadElems: Too few or too many"
						" nodes defining the volume element"<< endl;
		MPI_Abort(MPI_COMM_WORLD, 1);
	}

//	intVector pivotCol(2);
//	pivotCol[0] = 0;
//	pivotCol[1] = 2;
//
...
//	int Numface = 2;
//
//	for (int i = 0; i < pivotCol.size(); i++) {
//
//		//! Assemble 1st column of the faceMatrix which represents
//		//! the pivotal column
//		for (int j = 0; j < 2; j++)
//			faceMatrix[j][0] = surfaceElems[j][pivotCol[i]];
//
//		for (int k = 0; k < surfaceElems[0].size(); k++) {
//
//			//! Assemble 2nd column of the faceMatrix
//			if (k != pivotCol[0] && k != pivotCol[1] ) {
//				for (int l = 0; l < 2; l++)
//					faceMatrix[l][1] = surfaceElems[l][k];
//
//				//! Record the face nodes in the surfaceElems list
//				surfaceElems[Numface][0] = faceMatrix[0][0];
//				surfaceElems[Numface][1] = faceMatrix[0][1];
//				surfaceElems[Numface][2] = faceMatrix[1][1];
//				surfaceElems[Numface][3] = faceMatrix[1][0];
//
//				Numface++;
//
//			}
...
/*!****************************************************************************/
//!
intVector FEMGeometryX::surfaceIDGenerator(intMatrix surfaceNodesList,
       		InputFileData* InputData,std::map<std::string, double>& modelData,
       		std::ofstream& logFile){
...
/*!****************************************************************************/
//!
int FEMGeometryX::findSurfElemInSurfList(intVector& surfaceNodes,
		InputFileData* InputData, std::map<std::string, double>& modelData,
		std::ofstream& logFile) {
...
/*!****************************************************************************/
//!
int FEMGeometryX::storeSurfaceElem(intVector& surfaceNodes,
       		InputFileData* InputData,std::map<std::string, double>& modelData,
       		std::ofstream& logFile){
	...
	int usedDOF = (int) modelData["usedDegreesOfFreedom"];
	FEMElementX newSurfaceElem(usedDOF);
...
int FEMGeometryX::findPointInGeometry(dbVector& pointCoord,
		InputFileData* InputData, std::map<std::string, double>& modelData,
		std::ofstream& logFile) {
...
bool FEMGeometryX::findPointInVolumeElem(dbVector& pointCoord,
	...
	for (int i = 0; i < surfaceElems.size(); i++) {
		...
		if (surfaceLocation == -1) {
			logFile << "In FEMGeometryX::findPointInVolume: Surface element "
		...
		for (int j = 0; j < surfaceNodes.size(); j++) {
			...
			if (location == -1) {
				logFile << "In FEMGeometryX::findPointInVolumeElem: Particle not "
		...
		if (refNodeLocation != -1) {
			...
			logFile
					<< "In FEMGeometryX::findPointInVolume: Reference point not found in list"
					<< endl;
			MPI_Abort(MPI_COMM_WORLD, 1);
		}
...
int FEMGeometryX::findPtcleIDInPtcleList(int& ptcleID, std::ofstream& logFile) {
...
int FEMGeometryX::findSurfaceElemIDInList(int& surfaceID,
		std::ofstream& logFile) {
...
int FEMGeometryX::findVolumeElemIDInVolList(int& volumeID,
		std::ofstream& logFile) {
...
intVector FEMGeometryX::findAdjacentVolElemsOfVolumeElement(int& volID,
		std::ofstream& logFile) {
...
dbMatrix FEMGeometryX::setupLocalBasis(dbMatrix& surfacePointsCoords,
		InputFileData* InputData, std::map<std::string, double>& modelData,
		std::ofstream& logFile){
...
dbVector FEMGeometryX::caclSurfaceNormal(dbMatrix& surfacePointsCoords,InputFileData* InputData,
		std::ofstream& logFile){

	using namespace std;

	if(surfacePointsCoords.size() < 3){
		logFile <<"In FEMGeometryX::caclSurfaceNormal: Too few coordinates"
...
dbVector FEMGeometryX::normaliseVec(dbVector& vec, std::ofstream& logFile){
...
void FEMGeometryX::writeMeshFile(InputFileData* InputData, std::ofstream& logFile){
