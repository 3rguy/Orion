...
void BackgroundMesh::copyFEData(FEMGeometry* FEData,InputFileData* InputData,
  ...
  surfaceElectricBoundGaussPtsIdx =
    FEData->getSurfaceElectricBoundGaussPtsIdx();
  ...
  lineDepolarisationBoundGaussPtsIdx =
    FEData->getLineDepolarisationBoundGaussPtsIdx();
  pointDepolarisationBoundPtcleIdx =
    FEData->getPointDepolarisationBoundPtcleIdx();
  ...
  surfaceMomentBoundGaussPtsIdx = FEData->getSurfaceMomentBoundGaussPtsIdx();
  ...
  resizeArray(dummyIntMat,0);
  dummyIntMat = FEData->getElasticLineForceBoundGaussPtsIdx();
  resizeArray(dummyIntMat,0);
  ...
  if(dirichletIntegration) {
    ...
    for(int i = 0,j = oldSize;i < pointDispBoundPtcleIdx.size();i++,j++) {
      ...
      dbVector& gaussConds = boundGaussPoints[j].getDeformationBoundConds(
          condID);
    ...
    for(int i = 0,j = oldSize;i < pointDepolarisationBoundPtcleIdx.size();
        i++,j++) {
...
void BackgroundMesh::setPtcleRadsElemDepend(
    InputFileData* InputData,dbVector& globalInfluenceRadii,
  ...
  if(elementRootList.size() < globalLocalElemIdx.size()) {
    ...
    for(int i = 0;i < globalLocalElemIdx.size();i++)
      ...
      if(globalLocalElemIdx[i] >= 0) localElemRootList[i] = rank;

    MPI_Allreduce( &localElemRootList[0], &elementRootList[0],
                  globalLocalElemIdx.size(),MPI_INT,MPI_SUM,MPI_COMM_WORLD);
  ...
  for(int i = 0;i < particles.size();i++) {
    ...
    for(int j = 0;j < elements.size();j++) {
      ...
      for(int k = 0;k < nodes_j.size();k++) {
        ...
        if(distances.size() <= support) distances.resize(
            support + 1,dbVector(usedDims + 1));

        // coordinate x
        distances[support][0] = fabs(
            particles[ptcle].getCoord(0) - particles[i].getCoord(0));

        if(localInfluenceRadii[i * 3] < distances[support][0]) localInfluenceRadii[i
          * 3] = distances[support][0];

        // coordinate y
        distances[support][1] = fabs(
            particles[ptcle].getCoord(1) - particles[i].getCoord(1));

        if(localInfluenceRadii[i * 3 + 1] < distances[support][1]) localInfluenceRadii[i
          * 3 + 1] = distances[support][1];

        // coordinate z
        distances[support][2] = fabs(
            particles[ptcle].getCoord(2) - particles[i].getCoord(2));

        if(localInfluenceRadii[i * 3 + 2] < distances[support][2]) localInfluenceRadii[i
          * 3 + 2] = distances[support][2];
    ...
    // nodes
    bool motherElemsOnly = InputData->getValue(
        "setPtcleRadsElemDependMotherElemsOnly");
    int windowFuncShape = (int) InputData->getValue("windowFunctionShape");

    // spherical window functions (windowFuncShape = 2) only support
    // incorporation of direct neighbour particles
        ...
        for(int n = 0;n < nodes_n.size();n++) {
          ...
          for(int j = 0;j < neighElems.size();j++) {
            ...
            // other processes
              ...
              if(recvIdx[sourceIdx].size() > recvCounter[sourceIdx]) recvIdx[sourceIdx][recvCounter[sourceIdx]] =
                neighElems[j];

              else recvIdx[sourceIdx].push_back(neighElems[j]);
    ...
    for(int j = 0;j < size;j++) {
      ...
      MPI_Allgather( &recvCounter[j],1,MPI_INT, &recvCounts[0],1,MPI_INT,
                    MPI_COMM_WORLD);
      ...
      if(j == rank) {
        ...
        if(sendIdx.size() < sendIdxSize) sendIdx.resize(sendIdxSize);
          ...
          if(k == rank) continue;
            ...
            for(int l = 0;l < recvCounts[k];l++) {
              ...
              for(int r = 0;r < nodes_l.size();r++) {
                ...
                if(sendNodes[k].size() > sendCounter[k]) sendNodes[k][sendCounter[k]] =
                  nodes_l[r];

                else sendNodes[k].push_back(nodes_l[r]);
    ...
    for(int j = 0;j < size;j++) {
      ...
      MPI_Allgather( &sendCounter[j],1,MPI_INT, &recvCounts[0],1,MPI_INT,
                    MPI_COMM_WORLD);
      ...
      if(j == rank) {
        ...
        if(nonLocalNodes.size() < nonLocalNodesSize) nonLocalNodes.resize(
            nonLocalNodesSize);
    ...
    for(int j = 0;j < nonLocalNodesSize;j++) {
      ...
      if(distances.size() <= support) distances.resize(support + 1,
                                                       dbVector(usedDims + 1));
  ...
  if(x1Addend != 0 || x2Addend != 0 || x3Addend != 0) {
    ...
    for(int i = 0;i < particlesNum * 3;i += 3) {
      ...
      // 1-direction
      if(x1Addend != 0) globalInfluenceRadii[i] += x1Addend;

      // 2-direction
      if(x2Addend != 0) globalInfluenceRadii[i + 1] += x2Addend;

      // 3-direction
      if(x3Addend != 0) globalInfluenceRadii[i + 2] += x3Addend;
...
void BackgroundMesh::setPtcleRadsElemDepend2(
  ...
  for(int i = 0;i < particles.size();i++) {
    ...
    for(int j = 0;j < elements.size();j++) {
      ...
      for(int k = 0;k < nodes_j.size();k++) {
        ...
        // Check if current neighbour particle is already set.
        if(includedPtcls[ptcle]) continue;
              ...
              for(int k = 0;k < nodes_k.size();k++) {
                ...
                // not set yet
                  ...
                  // negative direction
                    ...
                    && localInfluenceRadii[negIdx]
                      < fabs(distances[support][0]))
...
int BackgroundMesh::getMaxGaussPtsPerVolumeElem() {
  ...
  if(maxGaussPtsPerVolumeElem <= 0) {
    ...
    for(int i = 0;i < nodesElements.size();i++) {
      ...
      if(gaussPtsPerElem > maxGaussPtsPerVolumeElem) maxGaussPtsPerVolumeElem =
        gaussPtsPerElem;
    }
...
void BackgroundMesh::setInfluenceRadii(InputFileData* InputData,
  ...
  case 2:
    ...
    setPtcleRadsSupportDepend(InputData,gaussPoints,boundGaussPoints,modelData,
                              logFile);
    ...
    for(int i = 0;i < particlesNum;i++) {
      ...
      MPI_Allreduce( &localInfRadii[0], &globalInfRadii[0],3,MPI_DOUBLE,MPI_MAX,
                    MPI_COMM_WORLD);
...
void BackgroundMesh::setInflSpheresConn(InputFileData* InputData,
  ...
  for(int i = 0;i < globalGaussPtsNum;i++) {
    ...
    MPI_Bcast( &supportingPtcls[i][0],suppCounts[i],MPI_INT,gaussRootList[i],
              MPI_COMM_WORLD);
  }
  ...
  for(int i = 0;i < localGaussPtsNum;i++) {
    ...
    for(int j = 0;j < supportingPtcls[globalGPIdx].size();j++) {
      ...
      for(int k = 0;k < supportingPtcls[globalGPIdx].size();k++) {
        ...
        if(inflSpheres.size() == 0) inflSpheres.push_back(
            supportingPtcls[globalGPIdx][k]);
        ...
        for(int l = 0;l < inflSpheres.size();l++) {
          ...
          if(supportingPtcls[globalGPIdx][k] == inflSpheres[l]) break;
          else if(supportingPtcls[globalGPIdx][k] < inflSpheres[l]) {
            inflSpheres.insert(inflSpheres.begin() + l,
                               supportingPtcls[globalGPIdx][k]);
            break;
          }
          else if(l + 1 == inflSpheres.size()) {
            inflSpheres.insert(inflSpheres.end(),
                               supportingPtcls[globalGPIdx][k]);
            break;
          }
      ...
      if(inflSpheres.size() > localMaxIndirectPtcleSupport) localMaxIndirectPtcleSupport =
        inflSpheres.size();
  ...
  for(int i = 0;i < globalBGaussPtsNum;i++) {
    ...
    MPI_Barrier(MPI_COMM_WORLD);
    MPI_Bcast( &supportingPtcls[i][0],suppCounts[i],MPI_INT,bGaussRootList[i],
              MPI_COMM_WORLD);
  ...
  for(int i = 0;i < localBGaussPtsNum;i++) {
    ...
    for(int j = 0;j < supportingPtcls[globalBGPIdx].size();j++) {
      ...
      for(int k = 0;k < supportingPtcls[globalBGPIdx].size();k++) {
        ...
        if(inflSpheres.size() == 0) inflSpheres.push_back(
            supportingPtcls[globalBGPIdx][k]);
        ...
        for(int l = 0;l < inflSpheres.size();l++) {
          ...
          if(supportingPtcls[globalBGPIdx][k] == inflSpheres[l]) break;
  ...
  for(int i = 0;i < particlesNum;i++) {
    ...
    MPI_Allgatherv( &inflSpheres[0],recvCounts[rank],MPI_INT,
                   &allGlobalSpheres[0], &recvCounts[0], &displs[0],MPI_INT,
                   MPI_COMM_WORLD);

    // extract the new influencing spheres
    removeRedundantEntries(allGlobalSpheres,0,allGlobalSpheresSize,logFile);
    resizeArray(allGlobalSpheres,allGlobalSpheresSize);
    ...
    if(maxIndirectPtcleSupport < allGlobalSpheresSize) maxIndirectPtcleSupport =
      allGlobalSpheresSize;
...
void BackgroundMesh::setPtclePtclsConn(InputFileData* InputData,
  ...
  if(mode == 1) {
    ...
    for(int i = 0;i < exclusiveLocalPtcls.size();i++) {
      ...
      setPointPtclsConn(InputData,coords,supportSize,suppPtcls,
                        localMinPtcleDist,modelData,logFile);
    ...
    for(int i = 0;i < particlesNum;i++) {
      ...
      MPI_Barrier(MPI_COMM_WORLD);
      MPI_Bcast( &particles[i].getLocalMinPtcleDistance(),1,MPI_DOUBLE,
  ...
  // the finite element mesh.
    ...
    blMatrix visitedBetaPtcls_(exclusiveLocalPtcls.size(),
                               blVector(particlesNum));
    dbMatrix tempVisitedBetaPtcls(particlesNum);
    bool visited;

    // Define Variables for finding localMinDistance
    double ptcleDist;
    dbVector minPtcleDistVec(particlesNum);
    ...
    for(int i = 0;i < exclusiveLocalPtcls.size();i++) {
      ...
      if(allSupportSize[alphaPtcle] < suppPtcls.size()) suppPtcls[allSupportSize[alphaPtcle]] =
        alphaPtcle;

      else suppPtcls.push_back(alphaPtcle);

      includedPtcls[alphaPtcle] = true;
      allSupportSize[alphaPtcle] += 1;

      if(neededElemCounter[alphaPtcle].size() == 0) neededElemCounter[alphaPtcle].resize(
          size);

      if(neededElemIdx[alphaPtcle].size() == 0) neededElemIdx[alphaPtcle].resize(
          size);
      ...
      for(int j = 0;j < alphaElems.size();j++) {
        ...
        if(localID >= 0) {
          ...
          for(int k = 0;k < betaNodes.size();k++) {
            ...
            if(includedElems[alphaElems[j]] == true) {
              setElemPtcleSupport(InputData,betaPtcle,includedPtcls,
                                  includedElems,allSupportSize[alphaPtcle],
                                  suppPtcls,coords,
                                  neededElemCounter[alphaPtcle],
                                  neededElemIdx[alphaPtcle],modelData,logFile);
            }

            // Verify that the Beta particle has not been
            // considered before or is not the alpha
            // particle itself !
            if(visitedBetaPtcls_[i][betaPtcle] == false
              && betaPtcle != alphaPtcle) {
              visitedBetaPtcls_[i][betaPtcle] = true;
              tempVisitedBetaPtcls[alphaPtcle].push_back(betaPtcle);
              if(particles[betaPtcle].querySupported(InputData,coords,ptcleDist,
                                                     modelData,logFile)) {
        ...
        // element not checked yet and nonlocal
          ...
          neededElemIdx[alphaPtcle][sourceIdx][neededElemCounter[alphaPtcle][sourceIdx]] =
            alphaElems[j];

          else neededElemIdx[alphaPtcle][sourceIdx].push_back(alphaElems[j]);
    ...
    while(globalNeededElems != 0) {
      ...
      for(int i = 0;i < particlesNum;i++) {
        ...
        flag = getNonLocalSupport(InputData,alphaPtcle,includedPtcls,
                                  neededElemCounter[alphaPtcle],
                                  neededElemIdx[alphaPtcle],nonLocalNodes,
                                  nonLocalBetaNodes,modelData,logFile);
        ...
        if(ptcleRootList[alphaPtcle] == rank) {
          ...
          for(int l = 0;l < nonLocalBetaNodes.size();l++) {
            ...
            if(visitedBetaPtcls_[exclusiveCounter][betaPtcle] == false
              && betaPtcle != alphaPtcle) {
        ...
        if(flag) {
          ...
          for(int k = 0;k < nonLocalNodes.size();k++) {
            ...
            setElemPtcleSupport(InputData,betaPtcle,includedPtcls,includedElems,
                                allSupportSize[alphaPtcle],suppPtcls,coords,
                                neededElemCounter[alphaPtcle],
                                neededElemIdx[alphaPtcle],modelData,logFile);
          }
          ...
          if(allSupportSize[alphaPtcle] < localMinSupport) localMinSupport =
            allSupportSize[alphaPtcle];
      ...
      MPI_Allreduce( &localNeededElems, &globalNeededElems,1,MPI_INT,MPI_SUM,
                    MPI_COMM_WORLD);
    ...
    for(int m = 0;m < particlesNum;m++) {
      ...
      if( !localMinPtcleDist[m].empty()) {
        ...
        // Use the average supporting particle distances
        particles[m].setLocalMinPtcleDistance(
            sum / localMinPtcleDist[m].size());

        else if(minPtcleDistMode == 2)
        // Use the minimum supporting particle distances
        particles[m].setLocalMinPtcleDistance(minPtcleDistVec[m]);
      }
    }
    ...
    for(int i = 0;i < particlesNum;i++) {
      ...
      MPI_Barrier(MPI_COMM_WORLD);
      MPI_Bcast( &supportSize,1,MPI_INT,ptcleRootList[i],MPI_COMM_WORLD);

      if(rank != ptcleRootList[i]) suppPtcls.resize(supportSize);
  ...
  if(rank == 0) cout << "particle support:    " << minPtcleSupport << " - "
  ...
  for(int i = 0;i < particlesNum;i++)
  logFile << "Ptcle " << i << ": " << particles[i].getLocalMinPtcleDistance()
  << endl;
#endif
#ifdef _supportDebugMode_
  if(rank == 0)
...
// finite element mesh.
void BackgroundMesh::setElemPtcleSupport(
    InputFileData* InputData,int& betaPtcle,blVector& includedPtcls,
    blVector& includedElems,int& supportSize,intVector& suppPtcls,
    dbVector& coords,intVector& neededElemCounter,intMatrix& neededElemIdx,
    std::map<std::string,double>& modelData,std::ofstream& logFile) {
  ...
  if(neededElemCounter.size() == 0) neededElemCounter.resize(size);

  if(neededElemIdx.size() == 0) neededElemIdx.resize(size);
  ...
  if( !includedPtcls[betaPtcle]
    && particles[betaPtcle].querySupported(InputData,coords,modelData,
                                           logFile)) {

    if(supportSize < suppPtcls.size()) suppPtcls[supportSize] = betaPtcle;

    else suppPtcls.push_back(betaPtcle);
    ...
    for(int i = 0;i < betaElems.size();i++) {
      ...
      if(localID >= 0 && !includedElems[betaElems[i]]) {
        ...
        for(int j = 0;j < gammaNodes.size();j++) {
          ...
          setElemPtcleSupport(InputData,gammaPtcle,includedPtcls,includedElems,
                              supportSize,suppPtcls,coords,neededElemCounter,
                              neededElemIdx,modelData,logFile);
      ...
      // element not checked yet and nonlocal
        ...
        if(neededElemIdx[sourceIdx].size() > neededElemCounter[sourceIdx]) neededElemIdx[sourceIdx][neededElemCounter[sourceIdx]] =
          betaElems[i];

        else neededElemIdx[sourceIdx].push_back(betaElems[i]);
...
bool BackgroundMesh::getNonLocalSupport(InputFileData* InputData,
  ...
  for(int i = 0;i < size;i++) {
    ...
    MPI_Allgather( &localCounts,1,MPI_INT, &recvCounts[0],1,MPI_INT,
                  MPI_COMM_WORLD);
  ...
  for(int i = 0;i < size;i++) {
    ...
    MPI_Allgather( &sendCounter[i],1,MPI_INT, &recvCounts[0],1,MPI_INT,
                  MPI_COMM_WORLD);
...
// Determine for a point a portion of nonlocal supporting particles.
bool BackgroundMesh::getNonLocalSupport(InputFileData* InputData,
                                        int& alphaPtcle,blVector& includedPtcls,
                                        intVector& neededElemCounter,
                                        intMatrix& neededElemIdx,
                                        intVector& nonLocalNodes,
          ...
          for(int l = 0;l < recvCounts[k];l++) {
            ...
            if(pos != -1) {
              ...
              for(int r = 0;r < nodes_l.size();r++) {
                ...
                if(sendBetaNodes[k].size() > sendBetaCounter[k]) sendBetaNodes[k][sendBetaCounter[k]] =
                  nodes_l[r];

                else sendBetaNodes[k].push_back(nodes_l[r]);
  ...
  for(int i = 0;i < size;i++) {
    ...
    MPI_Allgather( &sendBetaCounter[i],1,MPI_INT, &recvCounts[0],1,MPI_INT,
                  MPI_COMM_WORLD);
    ...
    if(i == rank) {
      ...
      if(nonLocalBetaNodes.size() < nonLocalBetaNodesSize) nonLocalBetaNodes.resize(
          nonLocalBetaNodesSize);
    ...
    MPI_Gatherv( &sendBetaNodes[i][0],recvCounts[rank],MPI_INT,
                &nonLocalBetaNodes[0], &recvCounts[0], &displs[0],MPI_INT,i,
                MPI_COMM_WORLD);
...
void BackgroundMesh::setPtcleGaussConn(InputFileData* InputData,
  ...
  MPI_Allgatherv( &iCounts[0],recvCounts[rank],MPI_INT, &inclPtsCounts[0],
  ...
  for(int j = 0;j < particlesNum;j++) {
    ...
    if(j != 0) inclPtsStartPos[j] = inclPtsStartPos[j - 1]
      + inclPtsCounts[j - 1];
    else inclPtsStartPos[j] = 0;
  }
  ...
  int globalIdxSize;
  MPI_Allreduce( &idxSize, &globalIdxSize,1,MPI_INT,MPI_SUM,MPI_COMM_WORLD);
  inclPts = intVector(globalIdxSize);

  MPI_Allgatherv( &iPoints[0],recvCounts[rank],MPI_INT, &inclPts[0],recvCounts,
                 displs,MPI_INT,MPI_COMM_WORLD);

#ifdef _geometryDebugMode_
  logFile << "******************** included points **********************"
  << endl;
  for(int i = 0;i < particlesNum;i++) {
    logFile << "PARTICLE " << i << ") ";
    for(int j = inclPtsStartPos[i];j < inclPtsStartPos[i] + inclPtsCounts[i];
        j++) {
      logFile << " " << inclPts[j];
    }
    logFile << endl;
...
void BackgroundMesh::setGaussPtcleConn(InputFileData* InputData,
  ...
  int gaussPtcleConnect = (int) InputData->getValue(
      "gaussParticleConnectivity");
  int mode = (int) InputData->getValue("supportComputationMode");
    ...
    for(int i = 0;i < localGaussPtsNum;i++) {
      ...
      if(supportCounts + suppGhostPtcls.size()
        ...
        logFile << "In BackgroundMesh::setGaussPtcleConn, too less particles\n"
            << "contain local gauss point " << i << " at processor \n" << rank
            << "( necessary: " << gaussPtcleConnect << " existing: "
            << supportCounts << ")!" << endl;
        MPI_Abort(MPI_COMM_WORLD,1);
      }

      if(supportCounts + suppGhostPtcls.size() > localMaxSupport) localMaxSupport =
        supportCounts;

      if(i == 0) localMinSupport = supportCounts;

      else if(supportCounts + suppGhostPtcls.size() < localMinSupport) localMinSupport =
        supportCounts;
  ...
  // element dependent support list computation
    ...
    for(int i = 0;i < localGaussPtsNum;i++) {
      ...
      for(int j = 0;j < betaNodes.size();j++) {
        ...
        setElemPtcleSupport(InputData,betaPtcle,includedPtcls,includedElems,
                            ...
                            neededElemCounter[i],neededElemIdx[i],modelData,
                            logFile);
      ...
      // free not needed memory
      for(int j = 0;j < size;j++)
        resizeArray(neededElemIdx[i][j],neededElemCounter[i][j]);
    ...
    MPI_Allreduce( &localGaussPtsNum, &maxLocalGaussPtsNum,1,MPI_INT,MPI_MAX,
                  MPI_COMM_WORLD);
      ...
      for(int i = 0;i < maxLocalGaussPtsNum;i++) {
        ...
        if(i < localGaussPtsNum) {
          ...
          flag = getNonLocalSupport(InputData,neededElemCounter[i],
                                    neededElemIdx[i],nonLocalNodes,modelData,
                                    logFile);
          ...
          if(flag) {
            ...
            for(int k = 0;k < nonLocalNodes.size();k++) {
              ...
              setElemPtcleSupport(InputData,betaPtcle,includedPtcls,
                                  includedElems,allSupportSize[i],suppPtcls,
                                  coords,neededElemCounter[i],neededElemIdx[i],
                                  modelData,logFile);
        ...
        // the other processes
          ...
          flag = getNonLocalSupport(InputData,dummyVec,dummyMat,nonLocalNodes,
                                    modelData,logFile);
    ...
    for(int i = 0;i < localGaussPtsNum;i++) {
      ...
      if(allSupportSize[i] + suppGhostPtcls.size()
        ...
        logFile << "In BackgroundMesh::setGaussPtcleConn, too less particles\n"
            << "contain local gauss point " << i << " at processor \n" << rank
            << "( necessary: " << gaussPtcleConnect << " existing: "
            << allSupportSize[i] + suppGhostPtcls.size() << ")!" << endl;
        MPI_Abort(MPI_COMM_WORLD,1);
      }

      if(allSupportSize[i] + suppGhostPtcls.size() > localMaxSupport) localMaxSupport =
        allSupportSize[i];

      if(i == 0) localMinSupport = allSupportSize[i];

      else if(allSupportSize[i] + suppGhostPtcls.size() < localMinSupport) localMinSupport =
        allSupportSize[i];
  ...
  // Determine global maximum number of supporting particles for a
  // gauss point.
  MPI_Allreduce( &localMaxSupport, &maxGaussSupport,1,MPI_INT,MPI_MAX,
                MPI_COMM_WORLD);

  // Determine global minimum number of supporting particles for a
  // gauss point.
  MPI_Allreduce( &localMinSupport, &minGaussSupport,1,MPI_INT,MPI_MIN,
                MPI_COMM_WORLD);

  logFile << "minGaussSupport = " << minGaussSupport << endl;
  logFile << "maxGaussSupport = " << maxGaussSupport << endl;

  if(rank == 0) cout << "Gauss-point support: " << minGaussSupport << " - "
      << maxGaussSupport << endl;
...
void BackgroundMesh::setBGaussPtcleConn(InputFileData* InputData,
  ...
  logFile << "#########################################################"
  << endl;
  logFile << "******* calculation of bound-gauss-ptcle connlist *******"
  << endl;
  ...
  // element dependent support list computation
    ...
    int maxLocalGaussPtsNum,localGlobalDifference;
    MPI_Allreduce( &localBGaussPtsNum, &maxLocalGaussPtsNum,1,MPI_INT,MPI_MAX,
                  MPI_COMM_WORLD);
    ...
    for(int i = 0;i < localBGaussPtsNum;i++) {
      ...
      if(i == 0) localMinSupport = minGaussSupport;
  ...
  // Determine global maximum number of supporting particles for a
  // gauss point.
  MPI_Allreduce( &localMaxSupport, &maxBGaussSupport,1,MPI_INT,MPI_MAX,
                MPI_COMM_WORLD);

  // Determine global minimum number of supporting particles for a
  // gauss point.
  MPI_Allreduce( &localMinSupport, &minBGaussSupport,1,MPI_INT,MPI_MIN,
                MPI_COMM_WORLD);

  // Check if the number of support boundary Gauss points is decisive
  if(maxBGaussSupport > maxGaussSupport) maxGaussSupport = maxBGaussSupport;

  if(minBGaussSupport < minGaussSupport) minGaussSupport = minBGaussSupport;

#ifdef _geometryDebugMode_
  logFile << "*********************************************************"
  << endl;
  logFile << "********* supporting particles of boundary gauss *********"
  << endl;
  for(int i = 0;i < localBGaussPtsNum;i++) {
    logFile << "Bound GAUSSPOINT " << i << ": ";
...
void BackgroundMesh::setLocalPtcls(InputFileData* InputData,
  ...
  if(localGlobalPtcls.size() > m) localGlobalPtcls.resize(m);
  ...
  for(int i = 0;i < allLocalPtcls[0].size();i++) {
    ...
    for(int j = 0;j < size;j++)
    if(allLocalPtcls[j][i] == 1) logFile << j << " ";
    logFile << "coords: ";
  ...
  if(exclusiveLocalPtcls.size() < particlesNum) exclusiveLocalPtcls.resize(
      particlesNum);
  ...
  if(algorithm == 1) {
    ...
    for(int i = 0;i < particlesNum;i++) {
      ...
      if(m == 1) {
        ...
        if(proc == rank) exclusiveLocalPtcls[localPortionCounts[proc]] = i;

        // determine the range
        if(i < minPtcle[proc]) minPtcle[proc] = i;

        if(i > maxPtcle[proc]) maxPtcle[proc] = i;
    ...
    for(int i = 0;i < particlesNum;i++) {
      ...
      for(int j = 0;j < size;j++) {
        ...
        if(allLocalPtcls[j][i] == 1 && localPortionCounts[j] < defaultPortion) {
          proc = j;
          break;
        }
  ...
  for(int j = 0;j < size;j++)
  logFile << "proc " << j << ": minPtcle=" << minPtcle[j] << " maxPtcle="
  << maxPtcle[j] << endl;
  logFile << "------------------------------------------------------" << endl;
  ...
  for(int i = 0;i < particlesNum;i++) {
    for(int j = size - 1;j >= 0;j--) {
      if(allLocalPtcls[j][i] == 1) logFile << "ptcle " << i << ": rank=" << j
      << endl;
    }
  }
#endif
  ...
  int distributingMode = (int) InputData->getValue(
      "parallelPtcleDistributingMode");
...
#ifdef _geometryDebugMode_
        if(allLocalPtcls[j][i] == 1) logFile << "ptcle " << i << ": rank=" << j
        << " centre=" << (minPtcle[j] + maxPtcle[j]) / 2.0 << " diff="
        << fabs(i - (minPtcle[j] + maxPtcle[j]) / 2.0) << endl;
#endif
  ...
  if(exclusiveLocalPtcls.size() == 0) {
    logFile
        << "In function ParticleDistribution::setLocalPtcls particle split\n"
  ...
  MPI_Allreduce( &localRootList[0], &ptcleRootList[0],particlesNum,MPI_INT,
                MPI_SUM,MPI_COMM_WORLD);

  int maxPtcleNum,minPtcleNum,ptcleSum;
  int localPtcleNum = exclusiveLocalPtcls.size();

  MPI_Allreduce( &localPtcleNum, &maxPtcleNum,1,MPI_INT,MPI_MAX,MPI_COMM_WORLD);
  MPI_Allreduce( &localPtcleNum, &minPtcleNum,1,MPI_INT,MPI_MIN,MPI_COMM_WORLD);

  MPI_Allreduce( &localPtcleNum, &ptcleSum,1,MPI_INT,MPI_SUM,MPI_COMM_WORLD);

  if(ptcleSum != particlesNum) {
    logFile << "In function BackgroundMesh::setLocalPtcls incomplete "
        << "particle split." << endl;
    if(rank == 0) cerr
        << "In function BackgroundMesh::setLocalPtcls incomplete "
        << "particle split." << endl;
    MPI_Abort(MPI_COMM_WORLD,1);
  }

  if(rank == 0) cout << "particle split: " << minPtcleNum << " - "
      << maxPtcleNum << endl;
  ...
  for(int i = 0;i < exclusiveLocalPtcls.size();i++) {
    ...
    else logFile << globalLocalPtcleIdx[exclusiveLocalPtcls[i]] << " ok"
    << endl;
  }
...
// vector ordering
void BackgroundMesh::rearrangeSupportLists(
...
void BackgroundMesh::assignGaussPtsDOF(InputFileData* InputData,
  ...
  for(int i = 0;i < localGaussPtsNum;i++) {
    ...
    intVector& suppPtcls = gaussPoints[i].getSupportPtcls();
    logFile << i << ".) GAUSSPOINT " << gaussPoints[i].getGlobalID() << ":size "
    << globalDof.size() << " ";
    for(int j = 0,k = 0;j < globalDof.size();j += usedDOF,k++) {
      if(globalDof[j] < lowestDOF) lowestDOF = globalDof[j];
      if(globalDof[j] > highestDOF) highestDOF = globalDof[j];
      logFile << suppPtcls[k] << ": ";
      for(int k = 0;k < usedDOF;k++)
...
void BackgroundMesh::setGaussDistribution(
    InputFileData* InputData,std::map<std::string,double>& modelData,
    std::ofstream& logFile) {
  ...
  if((int) InputData->getValue("partitionGaussPoints") == 1 && size > 1) {
    ...
    MPI_Allgather( &localBGaussPtsNum,1,MPI_INT, &bGaussPortions[0],1,MPI_INT,
                  MPI_COMM_WORLD);

    MPI_Allgather( &localGaussPtsNum,1,MPI_INT, &gaussPortions[0],1,MPI_INT,
                  MPI_COMM_WORLD);
    ...
    MPI_Allgatherv( &localBGIdx[0],recvCounts[rank],MPI_INT, &globalBGIdx[0],
                   recvCounts,displs,MPI_INT,MPI_COMM_WORLD);
    ...
    for(int i = 0;i < globalGaussPtsNum;i++)
      ...
      for(int j = 0;j < usedDims;j++)
        globalCoords[allIdx[i] * usedDims + j] = tmpGlobalCoords[i * usedDims
          + j];
    ...
    setPartitionGraph(InputData,gaussProcList,globalCoords,modelData,logFile);
  ...
  // Straight forward partitioning of volume and boundary Gauss points.
    ...
    if(rank > 0) {
      ...
      startPos = boundGaussPoints.begin();
    ...
    logFile << "global startIdx = " << startIdx << " endIdx = " << endIdx
    << endl;
    logFile << "localBGaussPtsNum = " << localBGaussPtsNum << " ?=  "
    ...
    for(int i = 0;i < globalLocalIdx.size();i++) {
      if(globalLocalIdx[i] != -1) logFile << i << ".) global: "
      << boundGaussPoints[m].getGlobalID() << " -> local: "
      << globalLocalIdx[i] << endl;
      else logFile << i << ".) " << globalLocalIdx[i] << " -> not available "
      << endl;
      m++;
    }
#endif
  ...
  if(localGaussPtsNum == 0) {
    logFile << "In function BackgroundMesh::setGaussDistribution volume "
        ...
        << "Gauss points." << endl << "Choose less processes." << endl;
  ...
  for(int i = 0;i < tmpGaussPtsIdx.size();i++) {
    ...
    if(globalLocalIdx[tmpGaussPtsIdx[i][0]] != -1) {
      int& ID = globalLocalIdx[tmpGaussPtsIdx[i][0]];
      cavityVolumeControlBoundGaussPtsIdx[m][0] = ID;

      for(int j = 1;j < tmpGaussPtsIdx[i].size();j++)
        cavityVolumeControlBoundGaussPtsIdx[m][j] = tmpGaussPtsIdx[i][j];
  ...
  cavityVolumeControlBoundGaussPtsIdx.resize(m);
  ...
  for(int i = 0;i < tmpGaussPtsIdx.size();i++)
    ...
    if(globalLocalIdx[tmpGaussPtsIdx[i][0]] != -1) {
      int& ID = globalLocalIdx[tmpGaussPtsIdx[i][0]];
      elasticSurfaceForceBoundGaussPtsIdx[m][0] = ID;

      for(int j = 1;j < tmpGaussPtsIdx[i].size();j++)
        elasticSurfaceForceBoundGaussPtsIdx[m][j] = tmpGaussPtsIdx[i][j];

      m++;
    }

  elasticSurfaceForceBoundGaussPtsIdx.resize(m);

  // elastic line force boundary conditions
  tmpGaussPtsIdx = elasticLineForceBoundGaussPtsIdx;
  m = 0;
  ...
  for(int i = 0;i < tmpGaussPtsIdx.size();i++)
    ...
    if(globalLocalIdx[tmpGaussPtsIdx[i][0]] != -1) {
      int& ID = globalLocalIdx[tmpGaussPtsIdx[i][0]];
      elasticLineForceBoundGaussPtsIdx[m][0] = ID;

      for(int j = 1;j < tmpGaussPtsIdx[i].size();j++)
        elasticLineForceBoundGaussPtsIdx[m][j] = tmpGaussPtsIdx[i][j];
...
#ifdef _geometryDebugMode_
  logFile << "****** local surface deformation boundary Gauss points *******"
  << endl;
  ...
  logFile << "****** local line deformation boundary Gauss points *******"
  << endl;
  ...
  logFile << "****** local point deformation boundary Gauss points *******"
  << endl;
  ...
  logFile << "****** local surface rotation boundary Gauss points *******"
  << endl;
  ...
  logFile << "****** local point rotation boundary Gauss points *******"
  << endl;
  ...
  logFile << "****** local surface electric boundary Gauss points *******"
  << endl;
  ...
  logFile << "****** local point electric boundary Gauss points *******"
  << endl;
  ...
  logFile << "****** local surface depolarisation boundary Gauss points *******"
  << endl;
  ...
  logFile << "****** local line depolarisation boundary Gauss points *******"
  << endl;
  ...
  logFile << "****** local point depolarisation boundary Gauss points *******"
  << endl;
  ...
  logFile << "****** local surface pressure boundary Gauss points *******"
  << endl;
  ...
  logFile << "*** local elastic line force boundary Gauss points ***" << endl;
  for(int i = 0;i < elasticLineForceBoundGaussPtsIdx.size();i++) {
    logFile << i << ".) ";
    for(int j = 0;j < elasticLineForceBoundGaussPtsIdx[i].size();j++)
    logFile << elasticLineForceBoundGaussPtsIdx[i][j] << " ";
    logFile << endl;
  }
  logFile << "*** local elastic surface force boundary Gauss points ***" << endl;
  for(int i = 0;i < elasticSurfaceForceBoundGaussPtsIdx.size();i++) {
    logFile << i << ".) ";
    for(int j = 0;j < elasticSurfaceForceBoundGaussPtsIdx[i].size();j++)
    logFile << elasticSurfaceForceBoundGaussPtsIdx[i][j] << " ";
    logFile << endl;
  }
  logFile << "****** local point force boundary Gauss points *******" << endl;
  ...
  logFile << "****** local surface moment boundary Gauss points *******"
  << endl;
  ...
  logFile << "***** local cavity-volume control boundary Gauss points ****"
  << endl;
  for(int i = 0;i < cavityVolumeControlBoundGaussPtsIdx.size();i++) {
    logFile << i << ".) ";
    for(int j = 0;j < cavityVolumeControlBoundGaussPtsIdx[i].size();j++)
  ...
  MPI_Allreduce( &localRootList[0], &gaussRootList[0],globalGaussPtsNum,MPI_INT,
                MPI_SUM,MPI_COMM_WORLD);
  ...
  if(globalBGaussPtsNum > globalGaussPtsNum) localRootList.resize(
      globalBGaussPtsNum);
  ...
  for(int i = 0;i < localBGaussPtsNum;i++)
    localRootList[boundGaussPoints[i].getGlobalID()] = rank;
  ...
  for(int i = 0;i < localGaussPtsNum;i++) {
    ...
    logFile << i << ".) Volume GAUSSPOINT " << gaussPoints[i].getGlobalID()
    << ": ";
  ...
  if(rank == 0) cout << "volume Gauss point split: " << minGaussPtsNum << " - "
      << maxGaussPtsNum << endl;
...
void BackgroundMesh::setPartitionGraph(InputFileData* InputData,
  ...
  setPtcleGaussConn(InputData,allGCoords,inclPtsStartPos,inclPtsCounts,inclPts,
                    modelData,logFile);
  ...
  getInitBGaussSplitting(InputData,defaultPortion,startIdx,endIdx,modelData,
                         logFile);

  // Calculate the reserved buffer space for the unreduced graph list.
  int gaussPtsPerElem = getMaxGaussPtsPerVolumeElem();

  int idxSize = (int) ((localGaussPtsNum + localBGaussPtsNum) * maxPtcleSupport
    * maxGaussSupport * gaussPtsPerElem
    / pow(maxPtcleSupport + maxGaussSupport,1.0 / 2.0));

#ifdef _geometryDebugMode_
  logFile << "*********************************************************"
  << endl;
  logFile << "******************** CONN LIST set up *******************"
  << endl;
  logFile << "localBGaussPtsNum = " << localBGaussPtsNum << endl;
  logFile << "startIdx: " << startIdx << "; endIdx: " << endIdx << endl;
  ...
  for(int i = 0;i < localGaussPtsNum;i++) {
    ...
    for(int j = 0;j < gaussPoints[i].getSupportCounts();j++) {
      ...
      for(int k = inclPtsStartPos[particle];
          k < inclPtsStartPos[particle] + inclPtsCounts[particle];k++) {
        ...
        if(indicator) {
          ...
          if(idxSize < oldIdxSize) tmpConnList[idxSize] = inclPts[k];
          else {
  ...
  logFile << "************************************" << endl;
  for(int i = localGaussPtsNum,k = startIdx;
      i < localGaussPtsNum + localBGaussPtsNum;i++,k++) {
  ...
  for(int i = startIdx;i < endIdx;i++) {
...
#ifdef _geometryDebugMode_
        logFile << "check " << idx << " " << idxSize << " " << inclPts[k]
        << endl;
#endif
    ...
    else cCounts[localGaussPtsNum + m] = idx;
  ...
  logFile << "reserved buffer for tmpConnlist: " << oldIdxSize << " -> needed: "
  << idxSize << endl;
#endif
  ...
  for(int i = 0;i < localGaussPtsNum + localBGaussPtsNum;i++)
    ...
    if(idx < cCounts[i]) idx = cCounts[i];
  ...
  for(int i = 0;i < localGaussPtsNum;i++) {
    ...
    while(k < cCounts[i]) {
      ...
      distance[k] = sqrt(
          pow(allGCoords[globIdx * 3] - allGCoords[point * 3],2)
    ...
    // points neighbours.
    cCounts[i] = (int) (ceil(connListMultiplier * cCounts[i]));
    if(cCounts[i] < 4) cCounts[i] = 4;

#ifdef _geometryDebugMode_
    logFile << i << " ************* write points in localConn **********"
    << endl;
#endif
  ...
  for(int i = startIdx;i < endIdx;i++) {
    ...
    // Determine for each gauss points the necessary number of gauss
    // points neighbours.
    cCounts[m] = (int) (ceil(connListMultiplier * cCounts[m]));
...
#ifdef _geometryDebugMode_
  logFile << "reserved buffer for connlist: " << oldIdxSize << " -> needed: "
  << idxSize << endl;
#endif
  ...
  for(int i = 0;i <= localGaussPtsNum + localBGaussPtsNum;i++) {
    ...
    if(i != 0) connListStartPos[i] = connListStartPos[i - 1] + cCounts[i - 1];
    else connListStartPos[i] = 0;
  }
  ...
  MPI_Allgather( &sendBuf,1,MPI_INT, &tmpRanges[0],1,MPI_INT,MPI_COMM_WORLD);
...
void BackgroundMesh::exchangeGaussData(InputFileData* InputData,
  ...
  MPI_Allgather( &allLocalGPtsNum,1,MPI_INT, &allLocalGPtsPortions[0],1,MPI_INT,
                MPI_COMM_WORLD);
  ...
  MPI_Allgather( &localGaussPtsNum,1,MPI_INT, &localGPtsPortions[0],1,MPI_INT,
                MPI_COMM_WORLD);
    ...
    for(int j = 0;j < localGPtsPortions[i];j++) {
      ...
      if(allOtherProcLists[gPoint] == rank) {
        ...
        if(gPoint < allLocalDispls[rank]
          ...
          if(recv < recvGaussIdx.size()) recvGaussIdx[recv] =
            globalVolGaussIdx[idx];
          else recvGaussIdx.push_back(globalVolGaussIdx[idx]);
  ...
  MPI_Allreduce( &sendIdx, &tmpGlobalSize,1,MPI_INT,MPI_SUM,MPI_COMM_WORLD);
  ...
  MPI_Allgatherv( &localGlobalBodyForceIdx[0],recvIdx[rank],MPI_INT,
                 &tmpGlobalBodyForceIdx[0], &recvIdx[0], &idxDispls[0],MPI_INT,
                 MPI_COMM_WORLD);
  ...
  logFile << "global int point " << i << " bool applied load "
  << globalBodyForceIdx[i] << endl;
#endif
  ...
  MPI_Allgatherv( &localGlobalBodyChargeIdx[0],recvIdx[rank],MPI_INT,
                 &tmpGlobalBodyChargeIdx[0], &recvIdx[0], &idxDispls[0],MPI_INT,
                 MPI_COMM_WORLD);

#ifdef _geometryDebugMode_
  logFile << "*********** old bodyElectricChargeGaussPtsIdx ****************"
  << endl;
  for(int i = 0;i < bodyElectricChargeGaussPtsIdx.size();i++)
  ...
  for(int i = 0;i < m;i++) {
    ...
    << sendCoords[i][0] << " " << sendCoords[i][1] << " "
    << sendCoords[i][2] << " weight " << sendWeights[i] << " loads ";
    for(int j = 0;j < sendBodyForces[i].size();j++)
    logFile << "DOF " << sendBodyForceDOF[i][j] << " cond "
    << sendBodyForces[i][j] << " ";
    logFile << endl;
    for(int j = 0;j < sendBodyCharges[i].size();j++)
    logFile << "DOF " << sendBodyChargeDOF[i][j] << " cond "
    << sendBodyCharges[i][j] << " ";
    logFile << endl;
  }
  ...
  if(newLocalGaussPtsNum != localGaussPtsNum) gaussPoints.resize(
      newLocalGaussPtsNum);
  ...
  while(m < sendGaussIdx.size() || m < recvGaussIdx.size()) {
    ...
    if(m < recvGaussIdx.size()) {
      ...
      // global index
      MPI_Irecv( &gaussPoints[gaussIdx].getGlobalID(),1,MPI_INT,MPI_ANY_SOURCE,
                recvGaussIdx[m] * gEE,MPI_COMM_WORLD, &reqs[sent + recvd]);
      recvd++;

      // coordinates
      MPI_Irecv( &gaussPoints[gaussIdx].getCoord(0),usedDims,MPI_DOUBLE,
                MPI_ANY_SOURCE,recvGaussIdx[m] * gEE + 1,MPI_COMM_WORLD,
                &reqs[sent + recvd]);
      recvd++;

      // weight
      MPI_Irecv( &gaussPoints[gaussIdx].getWeight(),1,MPI_DOUBLE,MPI_ANY_SOURCE,
                recvGaussIdx[m] * gEE + 2,MPI_COMM_WORLD, &reqs[sent + recvd]);
      recvd++;
      ...
      if(globalBodyForceIdx[recvGaussIdx[m]] == 1) {
        ...
        MPI_Irecv( &dummyDOF[0],usedDims,MPI_INT,MPI_ANY_SOURCE,
                  recvGaussIdx[m] * gEE + 3,MPI_COMM_WORLD,
                  &reqs[sent + recvd]);

        for(int k = 0;k < dummyDOF.size();k++)
          affectedDOF[k] = (bool) dummyDOF[k];

        MPI_Irecv( &conditions[0],usedDims,MPI_DOUBLE,MPI_ANY_SOURCE,
                  recvGaussIdx[m] * gEE + 3,MPI_COMM_WORLD,
                  &reqs[sent + recvd]);
      ...
      MPI_Irecv( &gaussPoints[gaussIdx].getMaterialID(),1,MPI_INT,
                MPI_ANY_SOURCE,recvGaussIdx[m] * gEE + 4,MPI_COMM_WORLD,
                &reqs[sent + recvd]);
      recvd++;
      ...
      if(globalBodyChargeIdx[recvGaussIdx[m]] == 1) {
        ...
        blVector& affectedDOF =
          gaussPoints[gaussIdx].getBodyElectricChargeDOF();
        ...
        MPI_Irecv( &dummyDOF[0],usedDims,MPI_INT,MPI_ANY_SOURCE,
                  recvGaussIdx[m] * gEE + 5,MPI_COMM_WORLD,
                  &reqs[sent + recvd]);
    ...
    if(m < sendGaussIdx.size()) {
      ...
      // global index
      MPI_Isend( &sendGaussIdx[m],1,MPI_INT,allOtherProcLists[sendGaussIdx[m]],
                sendGaussIdx[m] * gEE,MPI_COMM_WORLD, &reqs[sent + recvd]);
      sent++;

      // coordinates
      MPI_Isend( &sendCoords[m][0],usedDims,MPI_DOUBLE,
                allOtherProcLists[sendGaussIdx[m]],sendGaussIdx[m] * gEE + 1,
                MPI_COMM_WORLD, &reqs[sent + recvd]);
      sent++;

      // weight
      MPI_Isend( &sendWeights[m],1,MPI_DOUBLE,
                allOtherProcLists[sendGaussIdx[m]],sendGaussIdx[m] * gEE + 2,
                MPI_COMM_WORLD, &reqs[sent + recvd]);
      sent++;
      ...
      if(globalBodyForceIdx[sendGaussIdx[m]] == 1) {
        ...
        MPI_Isend( &sendBodyForceDOF[m][0],usedDims,MPI_INT,
                  allOtherProcLists[sendGaussIdx[m]],sendGaussIdx[m] * gEE + 3,
                  MPI_COMM_WORLD, &reqs[sent + recvd]);
      ...
      // material ID
      MPI_Isend( &sendGaussIdx[m],1,MPI_INT,allOtherProcLists[sendGaussIdx[m]],
                sendGaussIdx[m] * gEE + 4,MPI_COMM_WORLD, &reqs[sent + recvd]);
      sent++;
      ...
      if(globalBodyChargeIdx[sendGaussIdx[m]] == 1) {
        ...
        MPI_Isend( &sendBodyChargeDOF[m][0],usedDims,MPI_INT,
                  allOtherProcLists[sendGaussIdx[m]],sendGaussIdx[m] * gEE + 5,
                  MPI_COMM_WORLD, &reqs[sent + recvd]);
    ...
    << (sendGaussIdx.size() < recvGaussIdx.size() ?
        recvGaussIdx.size() : sendGaussIdx.size()) << " newGaussIdx "
    << gaussIdx << " sentCount " << sent << " receiveCount " << recvd
    << endl;
#endif
  ...
  logFile << "*********** local body elec charge integration points *****"
  << endl;
  for(int i = 0;i < bodyElectricChargeGaussPtsIdx.size();i++) {
...
/************************************************************************/
// Determine the initial boundary Gauss point distribution.
void BackgroundMesh::getInitBGaussSplitting(
    InputFileData* InputData,int& defaultPortion,int& startIdx,int& endIdx,
    std::map<std::string,double>& modelData,std::ofstream& logFile) {
...
void BackgroundMesh::splitBoundGaussPts(InputFileData* InputData,
  ...
  for(int i = 0;i < allOtherProcLists.size();i++) {
    ...
    if(allOtherProcLists[i] != rank) boundGaussPoints.erase(
        boundGaussPoints.begin() + m);
  ...
  for(int i = 0;i < globalLocalIdx.size();i++) {
    ...
    else logFile << "-1 -> local: " << globalLocalIdx[i] << endl;
  }
#endif
...
void BackgroundMesh::mergeLocalElemGaussVectors(
    InputFileData* InputData,dbVector& localVec,int gaussEntries,
    dbMatrix3& globalMatrix,std::map<std::string,double>& calcData,
    std::map<std::string,double>& modelData,std::ofstream& logFile) {
  ...
  int gaussPtsPerVolElem = backGroundMeshInfo["gaussPointsPerVolumeElement"];
  ...
  if(globalSize == 0) {
    ...
    MPI_Gatherv( &localIdx[0],localIdx.size(),MPI_INT, &globalIdx[0],
    ...
    if(rank == 0) {
      ...
      for(int i = 0;i < size;i++) {
        ...
        globalUnsortedElemIdx[i].assign(
            globalIdx.begin() + displs[i],
            globalIdx.begin() + displs[i] + recvCounts[i]);
  ...
  if(rank == 0) globalVec.resize(
      globalElemNum * gaussPtsPerVolElem * gaussEntries);
  ...
  MPI_Gatherv( &localVec[0],recvCounts[rank],MPI_DOUBLE, &globalVec[0],
              &recvCounts[0], &displs[0],MPI_DOUBLE,0,MPI_COMM_WORLD);
  ...
  if(rank == 0) {
    ...
    if(globalMatrix.size() == 0) allocateArray(globalMatrix,globalElemNum,
                                               gaussPtsPerVolElem,gaussEntries);
...
void BackgroundMesh::getAllBoundPenaltyParameters(
    InputFileData* InputData,dbVector& allPenaltyParams,
    std::map<std::string,double>& modelData,std::ofstream& logFile) {
  ...
  intMatrix& deformationBGaussPtsIdx = getAllDefBoundGaussPtsIdx(InputData,
                                                                 modelData,
                                                                 logFile);
  ...
  dbVector localPenaltyParams(
      (deformationBGaussPtsIdx.size() + electricBGaussPtsIdx.size()
  ...
  int electricEndDOF = electricStartDOF
    + (int) modelData["electricDegreesOfFreedom"];
  ...
  int depolarisationEndDOF = depolarisationStartDOF
    + (int) modelData["depolarisationDegreesOfFreedom"];
  ...
  MPI_Allreduce( &sendBuf, &numOfGlobalParams,1,MPI_INT,MPI_SUM,MPI_COMM_WORLD);
...
void BackgroundMesh::getAllGPointHistoryVariables(
    InputFileData* InputData,dbVector& allHistoryVariables,int& historySize,
    std::map<std::string,double>& modelData,std::ofstream& logFile) {
  ...
  int numOfGlobalData;
  MPI_Allreduce( &sendBuf, &numOfGlobalData,1,MPI_INT,MPI_SUM,MPI_COMM_WORLD);
  ...
  MPI_Gatherv( &localHistoryVariables[0],recvCounts[rank],MPI_DOUBLE,
              &allHistoryVariables[0], &recvCounts[0], &displs[0],MPI_DOUBLE,0,
              MPI_COMM_WORLD);
  ...
  MPI_Allreduce( &sendBuf, &numOfBGlobalData,1,MPI_INT,MPI_SUM,MPI_COMM_WORLD);
  ...
  // process '0' acts as root
  MPI_Gatherv( &localHistoryVariables[0],recvCounts[rank],MPI_DOUBLE,
              &allHistoryVariables[numOfGlobalData], &recvCounts[0], &displs[0],
...
intMatrix&
BackgroundMesh::getAllDispBoundGaussPtsIdx(
    InputFileData* InputData,std::map<std::string,double>& modelData,
    std::ofstream& logFile) {

  using namespace std;

  if(allDisplacementBoundGaussPtsIdx.size() > 0) return allDisplacementBoundGaussPtsIdx;
    ...
    if((bool) modelData["displacementConstraint"]) {
      ...
      pushBackVector(allDisplacementBoundGaussPtsIdx,lineDispBoundGaussPtsIdx);
      pushBackVector(allDisplacementBoundGaussPtsIdx,pointDispBoundPtcleIdx);
    }
...
// condition is applied (point,line,surface).
intMatrix&
BackgroundMesh::getAllRotBoundGaussPtsIdx(
    InputFileData* InputData,std::map<std::string,double>& modelData,
    std::ofstream& logFile) {

  using namespace std;

  if(allRotationBoundGaussPtsIdx.size() > 0) return allRotationBoundGaussPtsIdx;

  else {

    if((bool) modelData["rotationConstraint"]) {

      pushBackVector(allRotationBoundGaussPtsIdx,surfaceRotBoundGaussPtsIdx);
      pushBackVector(allRotationBoundGaussPtsIdx,lineRotBoundGaussPtsIdx);
      pushBackVector(allRotationBoundGaussPtsIdx,pointRotBoundPtcleIdx);
    }
...
intMatrix&
BackgroundMesh::getAllElectricBoundGaussPtsIdx(
    InputFileData* InputData,std::map<std::string,double>& modelData,
    std::ofstream& logFile) {

  using namespace std;

  if(allElectricBoundGaussPtsIdx.size() > 0) return allElectricBoundGaussPtsIdx;
    ...
    if((bool) modelData["electricConstraint"]) {
      ...
      pushBackVector(allElectricBoundGaussPtsIdx,
                     surfaceElectricBoundGaussPtsIdx);
      pushBackVector(allElectricBoundGaussPtsIdx,lineElectricBoundGaussPtsIdx);
      pushBackVector(allElectricBoundGaussPtsIdx,pointElectricBoundPtcleIdx);
    }
...
// condition is applied.
intMatrix&
BackgroundMesh::getAllDepolarisationBoundGaussPtsIdx(
    InputFileData* InputData,std::map<std::string,double>& modelData,
    std::ofstream& logFile) {

  using namespace std;

  if(allDepolarisationBoundGaussPtsIdx.size() > 0) return allDepolarisationBoundGaussPtsIdx;
...
#ifdef _geometryDebugMode_
    logFile << "***************************************************" << endl;
    logFile << "******** all depolarisation gauss point indices *********"
    << endl;
...
// condition is applied (point,line,surface).
intMatrix&
BackgroundMesh::getAllMicroBoundGaussPtsIdx(
    InputFileData* InputData,std::map<std::string,double>& modelData,
    std::ofstream& logFile) {

  using namespace std;

  if(allMicroBoundGaussPtsIdx.size() > 0) return allMicroBoundGaussPtsIdx;

  else {

    if((bool) modelData["microConstraint"]) {

      pushBackVector(allMicroBoundGaussPtsIdx,surfaceMicroBoundGaussPtsIdx);
      pushBackVector(allMicroBoundGaussPtsIdx,lineMicroBoundGaussPtsIdx);
...
// condition is applied.
intMatrix&
BackgroundMesh::getAllDefBoundGaussPtsIdx(
    InputFileData* InputData,std::map<std::string,double>& modelData,
    std::ofstream& logFile) {

  using namespace std;

  if(allDeformationBoundGaussPtsIdx.size() > 0) return allDeformationBoundGaussPtsIdx;
    ...
    if((bool) modelData["displacementConstraint"]) {
      ...
      pushBackVector(allDeformationBoundGaussPtsIdx,
                     surfaceDispBoundGaussPtsIdx);
      pushBackVector(allDeformationBoundGaussPtsIdx,lineDispBoundGaussPtsIdx);
      pushBackVector(allDeformationBoundGaussPtsIdx,pointDispBoundPtcleIdx);
    }
    ...
    if((bool) modelData["rotationConstraint"]) {
      ...
      for(int i = 0;i < surfaceRotBoundGaussPtsIdx.size();i++) {
        ...
        if(position != -1) allDeformationBoundGaussPtsIdx[i][3] = 2;
...
intMatrix&
BackgroundMesh::getAllForceBoundGaussPtsIdx(
    InputFileData* InputData,std::map<std::string,double>& modelData,
    std::ofstream& logFile) {

  using namespace std;

  if(allForceBoundGaussPtsIdx.size() > 0) return allForceBoundGaussPtsIdx;
    ...
    if((bool) modelData["lineForceLoad"]) pushBackVector(
        allForceBoundGaussPtsIdx,lineForceBoundGaussPtsIdx);

    if((bool) modelData["pointForceLoad"]) pushBackVector(
        allForceBoundGaussPtsIdx,pointForceBoundPtcleIdx);

    if((bool) modelData["surfaceMomentLoad"]) pushBackVector(
        allForceBoundGaussPtsIdx,surfaceMomentBoundGaussPtsIdx);

    if((bool) modelData["surfaceElectricChargeLoad"]) pushBackVector(
        allForceBoundGaussPtsIdx,surfaceElectricChargeBoundGaussPtsIdx);
...
intMatrix&
BackgroundMesh::getAllSurfaceGaussPtsIdx(std::ofstream& logFile) {
...
intMatrix&
BackgroundMesh::getAllLineGaussPtsIdx(std::ofstream& logFile) {
...
void BackgroundMesh::getAllGaussDeforms(InputFileData* InputData,
  ...
  MPI_Gatherv( &localDefs[0],localDefs.size(),MPI_DOUBLE, &tmpGlobalDefs[0],
  ...
  MPI_Gatherv( &idx[0],recvCounts[rank],MPI_INT, &allIdx[0],recvCounts,displs,
...
void BackgroundMesh::getIntPointRotation(
    InputFileData* InputData,GaussPoint& gPoint,dbVector& allDOF,
    dbVector& rotation,std::map<std::string,double>& modelData,
    std::ofstream& logFile) {
