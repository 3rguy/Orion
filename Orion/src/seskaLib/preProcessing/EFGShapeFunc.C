...
// shape functions.
void
EFGShapeFunc::calcShapes(InputFileData* InputData,int& supportSize,
  ...
  // prismatic window function
  if(windowFuncShape == 1) {
    WFuncSet = new PrismaticWindowFunctionSet(InputData,ptcls,sPtcls,x,y,z,
                                              supportSize,0,modelData,logFile);
  }
  // spherical window function
  else {
    dbVector coords(usedDims);
    coords[0] = x;
    coords[1] = y;
    coords[2] = z;
    WFuncSet = new SphericalWindowFunctionSet(InputData,ptcls,sPtcls,coords,
                                              supportSize,0,modelData,logFile);
  }
  ...
  if(shapeFuncs.size() == 0) resizeArray(shapeFuncs,supportSize);
...
EFGShapeFunc::calcShapes(InputFileData* InputData,int& supportSize,
  ...
  if(windowFuncShape == 1) {
    WFuncSet = new PrismaticWindowFunctionSet(InputData,ptcls,sPtcls,x,y,z,
                                              supportSize,derivOrder,modelData,
                                              logFile);
  }
  // spherical window function
  else {
  ...
  for(int i = 0;i < supportSize;i++) {
    ...
    for(int j = 0;j < linEQSize;j++)
      xDerivShapes[i] += P[i][j] * dBCalc[j] * W[i]
                                                 + P[i][j] * BCalc[j] * dWx[i];
...
EFGShapeFunc::calcShapes(InputFileData* InputData,int& supportSize,
  ...
  for(int i = 0;i < supportSize;i++) {
    ...
    for(int j = 0;j < linEQSize;j++)
      xDerivShapes[i] += P[i][j] * dxBCalc[j] * W[i]
                                                  + P[i][j] * BCalc[j] * dWx[i];
