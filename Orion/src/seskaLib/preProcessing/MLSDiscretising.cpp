...
void MLSDiscretising::setInfluenceRadii(InputFileData* InputData,
  ...
  // asymmetric shape function
//   if(shapeFuncType == 5) {

//     InputData->setValue("radiusDeterminationAlgorithm",6.0);
//   }
//   else if(shapeFuncType != 5 && radiusMethod == 6) {

//     InputData->setValue("radiusDeterminationAlgorithm",4.0);
//   }

  // particle distance influence influence radius determination
//   if(radiusMethod == 1 || radiusMethod == 5)

//     InputData->getValue("supportComputationMode",1.0);
  ...
  case 1:
    ...
    MLSGaussIntegral::setInfluenceRadii(InputData,modelData,logFile
					,viewerMPI,viewerSEQ);
  ...
  case 2:
    ...
    MLSPtcleIntegral::setInfluenceRadii(InputData,modelData,logFile
					,viewerMPI,viewerSEQ);
...
/**********************************************************************/
// Calculation of the MLS interpolanten with Gaussian quadrature.
void MLSDiscretising::calcGaussIntShapes(InputFileData* InputData,
					 std::map<std::string,double>& calcData,
					 std::map<std::string,double>& modelData,
					 std::ofstream& logFile,
					 PetscViewer& viewerMPI,
  ...
  if(!(bool)InputData->getValue("modifiedMLS"))

    MLSGaussIntegral::setAllShapeFuncs(InputData,calcData,modelData,
				       logFile,viewerMPI,viewerSEQ);

  
  else 

    ModifiedMLSGauss::setAllShapeFuncs(InputData,calcData,modelData,
				       logFile,viewerMPI,viewerSEQ);
...
/**********************************************************************/
// Calculation of the MLS interpolanten with particle integration.
void MLSDiscretising::calcPtcleIntShapes(InputFileData* InputData,
					 std::map<std::string,double>& calcData,
					 std::map<std::string,double>& modelData,
					 std::ofstream& logFile,
					 PetscViewer& viewerMPI,
  ...
  MLSPtcleIntegral::setShapeFuncsOnPtcls(InputData,calcData,modelData,
					 logFile,viewerMPI,viewerSEQ);
...
// vector ordering
void MLSDiscretising::rearrangeSupportLists(InputFileData* InputData,
  ...
  case 1:
    ...
    MLSGaussIntegral::rearrangeSupportLists(InputData,newGlobalPtcls,
					    modelData,logFile);
  ...
  case 2:
    ...
    MLSPtcleIntegral::rearrangeSupportLists(InputData,newGlobalPtcls,
					    modelData,logFile);
...
void MLSDiscretising::setTransformingShapes(InputFileData* InputData,
					    intMatrix& boundaryPtcls,
					    intMatrix& localBoundPtcls,
					    intVector& newDOFID,
					    std::map<std::string,double>& calcData,
					    std::map<std::string,double>& modelData,
					    std::ofstream& logFile,
  ...
  for(int i=0;i<usedDOF;i++) {
	...
	for(int k=0;k<sPtcls.size();k++) {
	  ...
	  if(allGlobalLocalIdx[sPtcls[k]] >= localStartIdx[rank] && 
	     allGlobalLocalIdx[sPtcls[k]] < localStartIdx[rank] + ptclePortion)
    ...
    createSparseParallelPETScMat(ptclePortion,ptclePortion,
 				 particlesNum,particlesNum,
 				 0,diagEntries,0,offDiagEntries,
 				 mOptions,A,logFile);
    ...
    // row
    for(int j=localStartIdx[rank];j<localStartIdx[rank]+ptclePortion;j++) {
	...
	MatSetValues(A,1,&j,sPtcls.size(),&matIdx[0],
		     &matValues[0],INSERT_VALUES);
    ...
    createParallelPETScSolver(A,solverData,solvingMethod,ksp,
			      pc,calcData,modelData,logFile);
...
//      MPI_Abort(MPI_COMM_WORLD,1);
	...
	if(!checkZero(XGlobal[idxK])) {
	  ...
	  if(newDOFID[ptcleJ*usedDOF+i] < 0) {
	    ...
	    if(shps.size() <= bSIdx[k])
	      shps.resize(shps.size()*2);
	...
	if(!checkZero(XGlobal[idxK])) {
	  ...
	  if(newDOFID[ptcleJ*usedDOF+i] < 0) {
	    ...
	    if(sPtcls.size() <= bIdx[k])
	      sPtcls.resize(sPtcls.size()*2);
	...
	if(maxBoundPtcleSupport < bIdx[k] + nBIdx[k])
	  maxBoundPtcleSupport = bIdx[k] + nBIdx[k];
...
void MLSDiscretising::getAllPtcleDOF(InputFileData* InputData,
				     dbVector& allDOF,
				     std::map<std::string,double>& calcData,
				     std::map<std::string,double>& modelData,
				     std::ofstream& logFile) {
...
/************************************************************************/
// Assemble all step degrees of freedom. 
void MLSDiscretising::getAllPtcleStepDOF(InputFileData* InputData,
					 dbVector& allStepDOF,
					 std::map<std::string,double>& calcData,
					 std::map<std::string,double>& modelData,
					 std::ofstream& logFile) {
  ...
  for(int i=0;i<exclusiveLocalPtcls.size();i++) {
    ...
    if(stepDOF.size() < usedDOF)
      stepDOF.resize(usedDOF);
...
void MLSDiscretising::getAllPtcleDefDOF(InputFileData* InputData,
  ...
  mergeLocalPtcleVectors(InputData,localAllDOF,defDOF,allDOF,calcData,
			 modelData,logFile);
...
void MLSDiscretising::getAllPtcleDispDOF(InputFileData* InputData,
					 dbVector& allDOF,
					 std::map<std::string,double>& calcData,
					 std::map<std::string,double>& modelData,
					 std::ofstream& logFile) {
...
intMatrix& MLSDiscretising::getPointForceBoundPtcleIdx(InputFileData* InputData,
						       std::map<std::string,double>& modelData,
						       std::ofstream& logFile) {
...
void MLSDiscretising::matrixFieldMLSApproximation(std::vector<Particle>& referencePoints,
  ...
  for(int i=0;i<points.size();i++) {
    ...
    EFGShapeFunc::calcShapes(InputData,support,suppPtcls,referencePoints,
			     coords[0],coords[1],coords[2],shapes,dummy,
			     data,logFile,viewerSEQ);
