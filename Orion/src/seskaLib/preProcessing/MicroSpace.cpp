...
dbMatrix& MicroSpace::getMicroLengths(InputFileData* InputData,std::ofstream& logFile) { 
...
dbVector& MicroSpace::getMicroRotationVec(InputFileData* InputData,std::ofstream& logFile) { 
...
void MicroSpace::setMicroVolRatio(InputFileData* InputData,std::ofstream& logFile) { 
  ...
  if(microVolRatio == 0)
    logFile<<"Warning: material ID "<<materialID<<" has a zero volume "
	   <<"fraction."<<endl;
...
// layers
void MicroSpace::setInitialMicroDirections(InputFileData* InputData,
                                           dbVector& coords,
					   dbMatrix& macroDirectors,
                                           int& macroMatID,
  ...
  int anisotropyType = (int)InputData->getMatValue(materialID,"anisotropyType");
  ...
  case 4:
    ...
    innerTensorProduct(microMetrics,dummyDirector,
		       initialMicroDirectors[0],false,logFile);
    break;
  ...
  case 5:
    ...
    if(type == 1){
      ...
      double pi = getPi();
      double x3 = coords[2];
      double gamma = (pi/4)*(-sin(4*pi*pow((x3/thickness - 0.5),3))+1) + alpha0;
  ...
  case 6:
	...
	+ unitVectors[2][2];
      ...
      crossProduct(initialMicroDirectors[2],initialMicroDirectors[0],sheetTangent);
  ...
  case 10:
      ...
      int meanDirection = (int)InputData->getMatValue(materialID,"meanDirection");
      allocateArray(initialMicroDirectors,1,usedDims);
      ...
      default:
          logFile<<"In MicroSpace::setMicroDirections anisotropyType 9, meanDirection"
...
void MicroSpace::setStructuralTensors(InputFileData* InputData,
  ...
  if(structuralTensors.size() != initialMicroDirectors.size())
    ...
    allocateArray(structuralTensors,initialMicroDirectors.size(),
		  usedDims,usedDims);
  ...
  for(int dir=0;dir<structuralTensors.size();dir++) 
	...
	structuralTensors[dir][i][j] = 
	  initialMicroDirectors[dir][i]*initialMicroDirectors[dir][j];
...
void MicroSpace::setMicroMetrics(InputFileData* InputData,
				 dbVector& coords,
				 std::map<std::string,double>& calcData,
				 std::map<std::string,double>& modelData,
				 std::ofstream& logFile) { 
