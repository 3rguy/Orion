...
ShepardShapeFunc::ShepardShapeFunc(InputFileData* InputData,int& supportSize,
                                   intVector& sPtcls,
                                   std::vector<Particle>& ptcls,double& x,
                                   double& y,double& z,
  ...
  switch(derivationOrder) {
    ...
    case 1:
      ...
      calcShape1stDerivs(InputData,supportSize,sPtcls,ptcls,x,y,z,modelData,
                         logFile,viewerSEQ);
    ...
    case 2:
      ...
      calcShape2ndDerivs(InputData,supportSize,sPtcls,ptcls,x,y,z,modelData,
                         logFile,viewerSEQ);
...
// shape functions.
void
ShepardShapeFunc::calcShapes(InputFileData* InputData,int& supportSize,
...
ShepardShapeFunc::calcShapes(InputFileData* InputData,int& supportSize,
                             intVector& sPtcls,std::vector<Particle>& ptcls,
                             double& x,double& y,double& z,dbVector& shapes,
                             int& basisTermNum,
                             std::map<std::string,double>& modelData,
                             std::ofstream& logFile,PetscViewer& viewerSEQ) {
...
// shape functions and their first order derivations.
void
ShepardShapeFunc::calcShape1stDerivs(InputFileData* InputData,int& supportSize,
                                     intVector& sPtcls,
                                     std::vector<Particle>& ptcls,double& x,
                                     double& y,double& z,
                                     std::map<std::string,double>& modelData,
...
ShepardShapeFunc::calcShapes(InputFileData* InputData,int& supportSize,
  ...
  calcShape1stDerivs(InputData,supportSize,sPtcls,ptcls,x,y,z,modelData,logFile,
                     viewerSEQ);
...
// shape functions and their first and second order derivations.
void
ShepardShapeFunc::calcShape2ndDerivs(InputFileData* InputData,int& supportSize,
                                     intVector& sPtcls,
                                     std::vector<Particle>& ptcls,double& x,
                                     double& y,double& z,
                                     std::map<std::string,double>& modelData,
  ...
  for(int i = 0;i < supportSize;i++) {
    ...
    secondDerivShapes[3][i] = (dWxy[i] * sumW2 - dWx[i] * sumW * sumDyW
        - dWy[i] * sumW * sumDxW + 2 * W[i] * sumDxW * sumDyW
        - W[i] * sumW * sumDxyW) / sumW3;

    secondDerivShapes[4][i] = (dWyz[i] * sumW2 - dWy[i] * sumW * sumDzW
        - dWz[i] * sumW * sumDyW + 2 * W[i] * sumDyW * sumDzW
        - W[i] * sumW * sumDyzW) / sumW3;

    secondDerivShapes[5][i] = (dWzx[i] * sumW2 - dWz[i] * sumW * sumDxW
        - dWx[i] * sumW * sumDzW + 2 * W[i] * sumDzW * sumDxW
        - W[i] * sumW * sumDzxW) / sumW3;
  }
...
ShepardShapeFunc::calcShapes(InputFileData* InputData,int& supportSize,
                             intVector& sPtcls,std::vector<Particle>& ptcls,
                             double& x,double& y,double& z,dbVector& shapes,
                             dbMatrix& dShapes,dbMatrix& d2Shapes,
                             std::map<std::string,double>& modelData,
                             std::ofstream& logFile,PetscViewer& viewerSEQ) {

  using namespace std;

  calcShape2ndDerivs(InputData,supportSize,sPtcls,ptcls,x,y,z,modelData,logFile,
                     viewerSEQ);
